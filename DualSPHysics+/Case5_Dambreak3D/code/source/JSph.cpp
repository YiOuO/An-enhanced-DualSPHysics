...
void JSph::LoadConfigParameters(const JXml *xml){
  ...
  DDTArray=false;
  //-Divergence cleaning methods configuration.
  if (eparms.Exists("DivClean")) {
      switch (eparms.GetValueInt("DivClean", true, 0)) {
      case 0:  TDivClean = DIV_NONE;     break;
      case 1:  TDivClean = DIV_VEM;      break;
      case 2:  TDivClean = DIV_VEM_HPDC; break;
      default: Run_Exceptioon("Divergence clean method is not valid.");
      }
  }
  //-VEM limiter
  VEMThreshold = eparms.GetValueFloat("VEMThreshold", true, 0.1f);
   //-Volume conservation configuration.
  if (eparms.Exists("VolConserve")) {
      switch (eparms.GetValueInt("VolConserve", true, 0)) {
      case 0:  TVolConserve = VOL_NONE;     break;
      case 1:  TVolConserve = VOL_VCS;  break;
      default: Run_Exceptioon("Volume conservation method is not valid.");
      }
  }

  //-Riemann stabilization term coefficient
  Beta = eparms.GetValueFloat("LimiterCoef", true, 15.0f);
...
void JSph::AddBasicArrays(JDataArrays &arrays,unsigned np,const tdouble3 *pos
  ,const unsigned *idp,const tfloat3 *vel,const float *rhop,const unsigned *pardet
  ,const tfloat3 *normalvec, const float *rhoover, const float *veldiv)const
{
  arrays.AddArray("Pos" ,np,pos);
  arrays.AddArray("Idp" ,np,idp);
  arrays.AddArray("Vel" ,np,vel);
  arrays.AddArray("Rhop",np,rhop);
  arrays.AddArray("ParType", np, pardet);
  arrays.AddArray("NormalVec", np, normalvec);
  arrays.AddArray("Rho_Rho0", np, rhoover);
  arrays.AddArray("VelDiv", np, veldiv);
}
