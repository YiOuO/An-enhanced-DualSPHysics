...
/// Reordena datos de particulas segun sortpart[].
//------------------------------------------------------------------------------
__global__ void KerSortDataParticles(unsigned n, unsigned pini, const unsigned* sortpart, const unsigned* a, unsigned* a2)
{
    const unsigned p = blockIdx.x * blockDim.x + threadIdx.x; //-Particle number.
    if (p < n) {
        const unsigned oldpos = (p < pini ? p : sortpart[p]);
...
/// Reordena datos de particulas segun sortpart[].
//------------------------------------------------------------------------------
__global__ void KerSortDataParticles(unsigned n, unsigned pini, const unsigned* sortpart, const tmatrix3d* a, tmatrix3d* a2)
{
    const unsigned p = blockIdx.x * blockDim.x + threadIdx.x; //-Particle number.
    if (p < n) {
        const unsigned oldpos = (p < pini ? p : sortpart[p]);
...
/// Reordena datos de particulas segun sortpart[].
//------------------------------------------------------------------------------
__global__ void KerSortDataParticles(unsigned n, unsigned pini, const unsigned* sortpart, const tmatrix3f* a, tmatrix3f* a2)
{
    const unsigned p = blockIdx.x * blockDim.x + threadIdx.x; //-Particle number.
    if (p < n) {
        const unsigned oldpos = (p < pini ? p : sortpart[p]);
...
/// Reordena datos de particulas segun sortpart.
//==============================================================================
void SortDataParticles(unsigned np, unsigned pini, const unsigned* sortpart, const unsigned* a, unsigned* a2) {
    if (np) {
        dim3 sgrid = GetSimpleGridSize(np, DIVBSIZE);
        KerSortDataParticles << <sgrid, DIVBSIZE >> > (np, pini, sortpart, a, a2);
    }
...
/// Reordena datos de particulas segun sortpart.
//==============================================================================
void SortDataParticles(unsigned np, unsigned pini, const unsigned* sortpart, const tmatrix3d* a, tmatrix3d* a2) {
    if (np) {
        dim3 sgrid = GetSimpleGridSize(np, DIVBSIZE);
        KerSortDataParticles << <sgrid, DIVBSIZE >> > (np, pini, sortpart, a, a2);
    }
...
/// Reordena datos de particulas segun sortpart.
//==============================================================================
void SortDataParticles(unsigned np, unsigned pini, const unsigned* sortpart, const tmatrix3f* a, tmatrix3f* a2) {
    if (np) {
        dim3 sgrid = GetSimpleGridSize(np, DIVBSIZE);
        KerSortDataParticles << <sgrid, DIVBSIZE >> > (np, pini, sortpart, a, a2);
    }
