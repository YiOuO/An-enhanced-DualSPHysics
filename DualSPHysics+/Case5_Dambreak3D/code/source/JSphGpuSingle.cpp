...
void JSphGpuSingle::ConfigDomain(){
  ...
  RunCellDivide(true);
  CoumputeRho0N0();
  //-Uploads constants include Rho0 N0 on the GPU.
  ConstantDataUp();
}
...
void JSphGpuSingle::RunCellDivide(bool updateperiodic){
  ...
  Timersg->TmStart(TMG_NlSortData,false);
  {
    ...
    unsigned* pardetg=ArraysGpu->ReserveUint();
    tmatrix3f* corrmatg=ArraysGpu->ReserveMatrix3f();
    float3*   normalvecg=ArraysGpu->ReserveFloat3();

    float*    rhobarg = ArraysGpu->ReserveFloat();

    CellDivSingle->SortBasicArrays(Idpg,Codeg,Dcellg,Posxyg,Poszg,Velrhopg,idpg,codeg,dcellg,posxyg,poszg,velrhopg);
    CellDivSingle->SortDataArrays(ParDetg,pardetg);
    CellDivSingle->SortDataArrays(CorrMatg,corrmatg);
    CellDivSingle->SortDataArrays(NormalVecg,normalvecg);
    CellDivSingle->SortDataArrays(Rhobarg, rhobarg);
    ...
    swap(ParDetg,pardetg);      ArraysGpu->Free(pardetg);
    swap(CorrMatg,corrmatg);    ArraysGpu->Free(corrmatg);
    swap(NormalVecg,normalvecg);ArraysGpu->Free(normalvecg);
    swap(Rhobarg, rhobarg);     ArraysGpu->Free(rhobarg);
  }
  if(TStep==STEP_Verlet){
  ...
  else if(TStep==STEP_Symplectic && (PosxyPreg || PoszPreg || VelrhopPreg || PsiPreg)){ //-In reality, only necessary in the corrector not the predictor step??? | En realidad solo es necesario en el divide del corrector, no en el predictor??? 
    ...
    if (TDivClean == DIV_VEM_HPDC) {
        float* psig2 = ArraysGpu->ReserveFloat();
        CellDivSingle->SortDataArrays(PsiPreg, psig2);
        swap(PsiPreg, psig2);        ArraysGpu->Free(psig2);
    }
  }
  ...
  if (Shifting){
      float4* shiftposfsg = ArraysGpu->ReserveFloat4();
      CellDivSingle->SortDataArrays(ShiftPosfsg, shiftposfsg);
      swap(ShiftPosfsg, shiftposfsg); ArraysGpu->Free(shiftposfsg);
  }

  if (TDivClean == DIV_VEM) {
      float3* acevemg = ArraysGpu->ReserveFloat3();
      CellDivSingle->SortDataArrays(AceVemg, acevemg);
      swap(AceVemg, acevemg); ArraysGpu->Free(acevemg);
  }
      ...
      float* psig = ArraysGpu->ReserveFloat();
      CellDivSingle->SortDataArrays(Psig, psig);
      swap(Psig, psig); ArraysGpu->Free(psig);
  }
  if (TVolConserve == VOL_VCS) {
        float* pressvcsg = ArraysGpu->ReserveFloat();
        float3* shiftvcsg = ArraysGpu->ReserveFloat3();
        CellDivSingle->SortDataArrays(PressVcsg,pressvcsg);
        CellDivSingle->SortDataArrays(ShiftVcsg, shiftvcsg);
        swap(PressVcsg, pressvcsg); ArraysGpu->Free(pressvcsg);
        swap(ShiftVcsg, shiftvcsg); ArraysGpu->Free(shiftvcsg);
  }
...
void JSphGpuSingle::Interaction_Forces(TpInterStep interstep){
  //if(TBoundary==BC_MDBC && (MdbcCorrector || interstep!=INTERSTEP_SymCorrector))MdbcBoundCorrection(); //-Boundary correction for mDBC.Dummy particles adopted here.
  InterStep=interstep;
  ...
  const StInterParmsg parms=StrInterParmsg(Simulate2D
    ...
    ,0,Nstep,SymplecticDtPre,DivData,Dcellg
    ,Posxyg,Poszg,PosCellg,Velrhopg,Idpg,Codeg
    ,FtoMasspg,SpsTaug,dengradcorr
    ,ViscDtg,Arg,Aceg,Deltag
    ,SpsGradvelg
    ,ShiftPosfsg
    ,CorrMatg,NormalVecg,ParDetg
    ,PressVcsg,ShiftVcsg,Rhobarg
    ,PressVemg,AceVemg,Psig,Apsig,VelDivg
    ,NULL,NULL);
  //-Renormalized matrix; Normal vector;Particle classification; VEM; VCS; OPS
  if(interstep != INTERSTEP_SymCorrector)cusph::PreInteraction(parms);
...
void JSphGpuSingle::Run(std::string appname,const JSphCfgRun *cfg,JLog2 *log){
  ...
  //PariclesInitial(); cudaMemcpy(Velrhopg, Velrhop, sizeof(float4) * Np, cudaMemcpyHostToDevice);//-gravity should be defined before configdomain
  FreePartsInit();
  UpdateMaxValues();
...
void JSphGpuSingle::SaveData(){
  ...
  JDataArrays arrays;
  AddBasicArrays(arrays,npsave,AuxPos,Idp,AuxVel,AuxRhop,Pardet,NormalVec, Rho_Rho0,VelDiv);
...
void JSphGpuSingle::CoumputeRho0N0(){
    float3* dengradcorr = NULL;
    const bool lamsps = (TVisco == VISCO_LaminarSPS);
    unsigned bsfluid = BlockSizes.forcesfluid;
    unsigned bsbound = BlockSizes.forcesbound;

    const StInterParmsg parms2 = StrInterParmsg(Simulate2D
        , Symmetry //<vs_syymmetry>
        , TKernel, FtMode
        , lamsps, TDensity, ShiftingMode
        , TDivClean, TVolConserve, TShiftingOPS
    ...
    cusph::ComputeValue(parms2,RhobarRef,NRef);
}
