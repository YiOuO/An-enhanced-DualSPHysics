...
protected:
  ...
  float4 *Velrhopg;
  unsigned *ParDetg;//-Particle Detection 0-wall 1-splash 2-free surface 3-free surface vicinity 4-inner 5-wall vicinity -Note slightly different from CPU
  tmatrix3f* CorrMatg;//-Corrective matrix
  float3 *NormalVecg;//-Normal vector

  float *PressVcsg;//-VCS pressure
  float3 *ShiftVcsg;//-VCS shifting vector
  float *Rhobarg;//-Kernel summation density -rho/rho_0

  float *PressVemg;//-VEM pressure
  float3 *AceVemg;//-VEM acceleration

  float* Psi; float* Psig; float* PsiPreg; float* Apsig;//-PSI in HPDC; Variables for compute step: SYMPLECTIC.
  
  float* VelDivg;//-Velocity divergence
