...
/// Calcula Shifting final para posicion de particulas.
//==============================================================================
void JSphShifting::RunGpuOPS(unsigned n, unsigned pini, double dt, const float4* velrhop
    , float4* shiftposfs, cudaStream_t stm)const
{
    const double coefumagn = -0.2 * KernelH * KernelH;
    const float maxdist = float(KernelH / 20); //-Max shifting distance permitted (recommended).
    //const double coefumagn = dt * ShiftCoef * KernelH;
    //const float coeftfs = (Simulate2D ? 2.0f : 3.0f) - ShiftTFS;
    //const float maxdist = float(Dp * 0.1); //-Max shifting distance permitted (recommended).
    cushift::RunShiftingOPS(n, pini, dt, coefumagn, maxdist, velrhop, shiftposfs, stm);
}
#endif
