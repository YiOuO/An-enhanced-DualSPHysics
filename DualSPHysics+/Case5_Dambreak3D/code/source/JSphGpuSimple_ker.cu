...
template<bool floating,bool shift,bool inout> __global__ void KerComputeStepSymplecticPre
  ...
  ,typecode *code,double2 *movxy,double *movz,float4 *velrhop,const bool hpdc, const float* apsi, const float* psipre, float* psi)
{
  unsigned p=blockIdx.x*blockDim.x + threadIdx.x; //-Number of particle.
  if(p<n){
    if(p<npb){ //-Particles: Fixed & Moving.
      ...
      //-HPDC Calculate \psi
      if(hpdc){
          const float rpsipre = psipre[p];
          const float apsip1 = apsi[p];
          //-dPsi\dt
          float  rpsinew = float(double(rpsipre) + double(apsip1) * dtm);
          psi[p] = rpsinew;
      }
  ...
  ,typecode *code,double2 *movxy,double *movz,float4 *velrhop, const float* psipre, float* psi, const float* apsi,cudaStream_t stm)
{
  if(np){
    ...
    if(inout){      const bool tinout=true;
      if(shift){    const bool shift=true;
        if(floating)KerComputeStepSymplecticPre<true ,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,ar,ace,shiftposfs,indirvel,dtm,rhopzero,rhopoutmin,rhopoutmax,Float3(gravity),code,movxy,movz,velrhop,hpdc,apsi,psipre,psi);
        else        KerComputeStepSymplecticPre<false,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,ar,ace,shiftposfs,indirvel,dtm,rhopzero,rhopoutmin,rhopoutmax,Float3(gravity),code,movxy,movz,velrhop,hpdc,apsi,psipre,psi);
      }else{        const bool shift=false;
        if(floating)KerComputeStepSymplecticPre<true ,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,ar,ace,shiftposfs,indirvel,dtm,rhopzero,rhopoutmin,rhopoutmax,Float3(gravity),code,movxy,movz,velrhop,hpdc,apsi,psipre,psi);
        else        KerComputeStepSymplecticPre<false,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,ar,ace,shiftposfs,indirvel,dtm,rhopzero,rhopoutmin,rhopoutmax,Float3(gravity),code,movxy,movz,velrhop,hpdc,apsi,psipre,psi);
      }
...
template<bool floating,bool shift,bool inout> __global__ void KerComputeStepSymplecticCor
  (unsigned n,unsigned npb
  ,const float4 *velrhoppre,const float *ar,const float3 *ace,const float4 *shiftposfs,const bool vem,const float3 *acevem,const float vemthreshold, const bool vcs, const float3 *shiftvcs
  ,const float3 *indirvel,double dtm,double dt,float rhopzero,float rhopoutmin,float rhopoutmax,float3 gravity
  ,typecode *code,double2 *movxy,double *movz,float4 *velrhop,const bool hpdc, const float* psipre, float* psi, const float* apsi)
{
  unsigned p=blockIdx.x*blockDim.x + threadIdx.x; //-Number of particle.
  if(p<n){
    if(p<npb){ //-Particles: Fixed & Moving.
      ...
      rvelrhopnew.w = float(double(rvelrhoppre.w) + double(ar[p]) * dt);
      if(!floating || CODE_IsFluid(rcode)){//-Particles: Fluid.
        //-Calculate velocity. | Calcula velocidad.
        float3 race=ace[p];
        if (vem) {
            float normace = sqrtf(ace[p].x * ace[p].x + ace[p].y * ace[p].y + ace[p].z * ace[p].z);
            float normacevem = sqrtf(acevem[p].x * acevem[p].x + acevem[p].y * acevem[p].y + acevem[p].z * acevem[p].z);
            float maxacevem = (vemthreshold == 0) ? FLT_MAX : vemthreshold * normace;
            if (normacevem > maxacevem) {
                race.x += maxacevem * acevem[p].x/normacevem; race.y += maxacevem * acevem[p].y / normacevem; race.z += maxacevem * acevem[p].z / normacevem;
            }
            else{
                race.x += acevem[p].x; race.y += acevem[p].y; race.z += acevem[p].z;
            }
        }
        //const float3 race = vem? make_float3(ace[p].x+acevem[p].x, ace[p].y + acevem[p].y, ace[p].z + acevem[p].z):ace[p];
        ...
        if(vcs){
          const float3 rshiftvcs=shiftvcs[p];
          dx+=double(rshiftvcs.x);
          dy+=double(rshiftvcs.y);
          dz+=double(rshiftvcs.z);
        }
        if(hpdc){
            psi[p] = float(double(psipre[p]) + double(apsi[p]) * dt);
        }
  ...
  ,typecode *code,double2 *movxy,double *movz,float4 *velrhop, const float* psipre, float* psi, const float* apsi, cudaStream_t stm)
{
  if(np){
    ...
    if(inout){      const bool tinout=true;
      if(shift){    const bool shift=true;
        if(floating)KerComputeStepSymplecticCor<true ,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,ar,ace,shiftposfs,vem,acevem,vemthreshold,vcs,shiftvcs,indirvel,dtm,dt,rhopzero,rhopoutmin,rhopoutmax,Float3(gravity),code,movxy,movz,velrhop,hpdc, psipre, psi, apsi);
        else        KerComputeStepSymplecticCor<false,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,ar,ace,shiftposfs,vem,acevem,vemthreshold,vcs,shiftvcs,indirvel,dtm,dt,rhopzero,rhopoutmin,rhopoutmax,Float3(gravity),code,movxy,movz,velrhop,hpdc, psipre, psi, apsi);
      }else{        const bool shift=false;
        if(floating)KerComputeStepSymplecticCor<true ,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,ar,ace,shiftposfs,vem,acevem,vemthreshold,vcs,shiftvcs,indirvel,dtm,dt,rhopzero,rhopoutmin,rhopoutmax,Float3(gravity),code,movxy,movz,velrhop,hpdc, psipre, psi, apsi);
        else        KerComputeStepSymplecticCor<false,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,ar,ace,shiftposfs,vem,acevem,vemthreshold,vcs,shiftvcs,indirvel,dtm,dt,rhopzero,rhopoutmin,rhopoutmax,Float3(gravity),code,movxy,movz,velrhop,hpdc, psipre, psi, apsi);
      }
