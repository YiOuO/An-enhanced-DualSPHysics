...
JSphGpu::JSphGpu(bool withmpi):JSph(false,false,withmpi),DivAxis(MGDIV_None){
  ...
  AuxPos=NULL; AuxVel=NULL; AuxRhop=NULL;NormalVec=NULL;Pardet=NULL;Rho_Rho0=NULL;VelDiv=NULL;
  CellDiv=NULL;
...
void JSphGpu::InitVars(){
  ...
  ViscDtg=NULL; 
  Arg=NULL; Aceg=NULL; Deltag=NULL; ParDetg=NULL; CorrMatg=NULL;NormalVecg=NULL;
  PressVemg=NULL; AceVemg=NULL; Psig=NULL; PsiPreg=NULL;Apsig=NULL;VelDivg=NULL;
  PressVcsg=NULL; ShiftVcsg=NULL;
...
void JSphGpu::AllocCpuMemoryParticles(unsigned np){
  ...
  if(np>0){
    try{
      ...
      Pardet=new unsigned[np];   MemCpuParticles+=sizeof(unsigned)*np;
      NormalVec=new tfloat3[np]; MemCpuParticles+=sizeof(tfloat3)*np;
      Rho_Rho0=new float[np];    MemCpuParticles+=sizeof(float)*np;
      VelDiv=new float[np];      MemCpuParticles+=sizeof(float)*np;
    }
    catch(const std::bad_alloc){
...
void JSphGpu::AllocGpuMemoryParticles(unsigned np,float over){
  ...
  #endif
  //ArraysGpu->AddArrayCount(JArraysGpu::SIZE_4B,9);  //-idp,ar,viscdt,dcell,pressvcs,pressvem,psi,apsi,psipre-------
  ArraysGpu->AddArrayCount(JArraysGpu::SIZE_4B, 4);  //-idp,ar,viscdt,dcell
  ArraysGpu->AddArrayCount(JArraysGpu::SIZE_4B,3);  //-pardet,rhobar,veldiv 
  ArraysGpu->AddArrayCount(JArraysGpu::SIZE_4B,7);  //-extra
  ArraysGpu->AddArrayCount(JArraysGpu::SIZE_36B,2); //-corrmat
  //ArraysGpu->AddArrayCount(JArraysGpu::SIZE_12B,5); //-normalvec,shiftvcs,acevem,2*extra
  ArraysGpu->AddArrayCount(JArraysGpu::SIZE_12B,3); //-normalvec,2*extra
  ...
  if(TStep==STEP_Verlet){
    ...
    ArraysGpu->AddArrayCount(JArraysGpu::SIZE_4B, 1); //-psipre
  }
  ...
  //ArraysGpu->AddArrayCount(JArraysGpu::SIZE_16B, 2); //-shiftposfs,extra
  if(UseNormals){
  ...
  if (TDivClean == DIV_VEM) {
      ArraysGpu->AddArrayCount(JArraysGpu::SIZE_4B, 1); //-VEM Pressure
      ArraysGpu->AddArrayCount(JArraysGpu::SIZE_12B, 1);//-VEM Acceleration
  }
  else if (TDivClean == DIV_VEM_HPDC) {
      ArraysGpu->AddArrayCount(JArraysGpu::SIZE_4B, 4); //-Psi,Psipre,APsi,VEM Pressure
      ArraysGpu->AddArrayCount(JArraysGpu::SIZE_12B, 1);//-VEM Acceleration
  }
  if (TVolConserve == VOL_VCS) {
      ArraysGpu->AddArrayCount(JArraysGpu::SIZE_12B, 1);//-Shifting(VCS)
      ArraysGpu->AddArrayCount(JArraysGpu::SIZE_4B, 1);//-VCS Press
  }
...
void JSphGpu::ReserveBasicArraysGpu(){
  ...
  PosCellg=ArraysGpu->ReserveFloat4();
  Velrhopg=ArraysGpu->ReserveFloat4();
  ParDetg= ArraysGpu->ReserveUint();
  CorrMatg=ArraysGpu->ReserveMatrix3f();
  NormalVecg=ArraysGpu->ReserveFloat3();
  VelDivg = ArraysGpu->ReserveFloat();
  Rhobarg= ArraysGpu->ReserveFloat();
  if (Shifting) {
      ShiftPosfsg= ArraysGpu->ReserveFloat4();
  }
  ...
  if (TDivClean == DIV_VEM) {
      PressVemg = ArraysGpu->ReserveFloat();
      AceVemg = ArraysGpu->ReserveFloat3();
  }
  else if (TDivClean == DIV_VEM_HPDC) {
      PressVemg = ArraysGpu->ReserveFloat();
      AceVemg = ArraysGpu->ReserveFloat3();
      Psig = ArraysGpu->ReserveFloat();
      Apsig = ArraysGpu->ReserveFloat();
  }
  if (TVolConserve == VOL_VCS) {
      PressVcsg = ArraysGpu->ReserveFloat();
      ShiftVcsg = ArraysGpu->ReserveFloat3();
  }
}
...
void JSphGpu::ConstantDataUp(){
  ...
  ctes.rhobarzero=RhobarRef;ctes.nzero=NRef;
...
unsigned JSphGpu::ParticlesDataDown(unsigned n,unsigned pini,bool code,bool onlynormal){
  ...
  cudaMemcpy(Pardet, ParDetg + pini, sizeof(unsigned) * n, cudaMemcpyDeviceToHost);
  cudaMemcpy(NormalVec, NormalVecg + pini, sizeof(float3) * n, cudaMemcpyDeviceToHost);
  cudaMemcpy(Rho_Rho0, Rhobarg + pini, sizeof(float) * n, cudaMemcpyDeviceToHost);
  cudaMemcpy(VelDiv, VelDivg + pini, sizeof(float) * n, cudaMemcpyDeviceToHost);
  for (auto p = 0; p < n; p++)
  {
      Rho_Rho0[p] = Rho_Rho0[p]/RhobarRef;
  }
  ...
  if(CellMode==CELLMODE_Full || CellMode==CELLMODE_Half){
    ...
    memset(&divdatag,0,sizeof(StDivDataGpu));
      ...
      const StInterParmsg parms=StrInterParmsg(Simulate2D
        ...
        ,TDivClean,TVolConserve,TShiftingOPS
        ,0,0,0,0,100,0,0
        ,0,0,0,divdatag,NULL
...
void JSphGpu::InitRunGpu(){
  ...
  TShiftingOPS = Shifting?OPS:OPS_NONE;
  if(TDivClean==DIV_VEM_HPDC)cudaMemset(Psig, 0, sizeof(float) * Np);
  if(TVolConserve == VOL_VCS)cudaMemset(PressVcsg, 0, sizeof(float) * Np);//-Pressvcs Initialize
...
void JSphGpu::PreInteractionVars_Forces(unsigned np,unsigned npb){
  ...
  //if(ShiftPosfsg)cudaMemset(ShiftPosfsg, 0, sizeof(float4) * Np);
...
void JSphGpu::PreInteraction_Forces(){
  ...
  if(TDivClean==DIV_VEM_HPDC)cudaMemset(Apsig,0,sizeof(float)*Np);//-Apsig[]=0,0,0
  //-Initialize memory at Predictor step
  if (InterStep != INTERSTEP_SymCorrector)
  {
      cudaMemset(CorrMatg, 0, sizeof(tmatrix3f) * Np);//-Corrmatg[]=[0,0,0;0,0,0;0,0,0]
      cudaMemset(ParDetg, 0, sizeof(unsigned) * Np);//-ParDetg[]=0
      cudaMemset(NormalVecg+Npb, 0,sizeof(float3) * (Np-Npb));//-Reset normal vecs for fluid particles
      //cudaMemset(NormalVecg, 0, sizeof(float3) * (Np));
      cudaMemset(VelDivg, 0, sizeof(float) * Np);//-VelDivg[]=0
      cudaMemset(Rhobarg, 0, sizeof(float) * Np);//-Rhobarg[]=0
      if(Shifting)cudaMemset(ShiftPosfsg,0,sizeof(float4)*Np);//-ShiftPosfsg[]=0,0,0,0

      if (TDivClean)
      {
          cudaMemset(PressVemg, 0, sizeof(float) * Np);//-PressVemg[]=0
          cudaMemset(AceVemg, 0, sizeof(float3) * Np);//-AceVemg[]=0
      }
      if(TVolConserve)cudaMemset(ShiftVcsg, 0, sizeof(float3) * Np);//-ShiftVcsg[]=0,0,0
  }
...
void JSphGpu::ComputeSymplecticPre(double dt){
  ...
  const bool inout=(InOut!=NULL);
  const bool HPDC=(TDivClean == DIV_VEM_HPDC);
  ...
  if(HPDC){PsiPreg=ArraysGpu->ReserveFloat(); cudaMemset(PsiPreg, 0, sizeof(float) * Np); swap(PsiPreg,Psig);}//-Psipre[] <= Psi[]
  ...
  const float3 *indirvel=(InOut? InOut->GetDirVelg(): NULL);
  cusphs::ComputeStepSymplecticPre(WithFloating,shift,inout,HPDC,Np,Npb,VelrhopPreg,Arg
    ,Aceg,ShiftPosfsg,indirvel,dt05,RhopZero,RhopOutMin,RhopOutMax,Gravity
    ,Codeg,movxyg,movzg,Velrhopg, PsiPreg, Psig, Apsig,NULL);
...
void JSphGpu::ComputeSymplecticCorr(double dt){
  Timersg->TmStart(TMG_SuComputeStep,false);
  //const bool shift=(ShiftingMode!=SHIFT_None);
  const bool shift=(TShiftingOPS!=OPS_NONE);
  const bool inout=(InOut!=NULL);
  const bool VEM=(TDivClean!=DIV_NONE);
  const bool HPDC=(TDivClean==DIV_VEM_HPDC);
  const bool VCS=(TVolConserve!=VOL_NONE);
  ...
  const double dt05=dt*.5;
  const float3 *indirvel=(InOut? InOut->GetDirVelg(): NULL);
  cusphs::ComputeStepSymplecticCor(WithFloating,shift,inout,VEM,VCS,HPDC,Np,Npb,VelrhopPreg
    ,Arg,Aceg,ShiftPosfsg,AceVemg,VEMThreshold,ShiftVcsg,indirvel,dt05,dt,RhopZero,RhopOutMin,RhopOutMax,Gravity
    ,Codeg,movxyg,movzg,Velrhopg,PsiPreg,Psig,Apsig,NULL);
  ...
  if(HPDC)ArraysGpu->Free(PsiPreg);PsiPreg = NULL;
  Timersg->TmStop(TMG_SuComputeStep,false);
}
...
void JSphGpu::RunShifting(double dt){
  ...
  Shifting->RunGpuOPS(Np-Npb,Npb,dt,Velrhopg,ShiftPosfsg);
...
void JSphGpu::RunMotion(double stepdt){
  ...
  float3 *normalvec=NULL;
  normalvec=NormalVecg;
  ...
  if(DsMotion->GetActiveMotion()){
    ...
    for(unsigned ref=0;ref<nref;ref++){
      ...
      if(m.type==MOTT_Matrix){//-Matrix movement (for rotations).
        if(motsim)cusph::MoveMatBound   (PeriActive,Simulate2D,m.count,m.idbegin-CaseNfixed,m.matmov,stepdt,RidpMoveg,Posxyg,Poszg,Dcellg,Velrhopg,Codeg,boundnormal,normalvec);
