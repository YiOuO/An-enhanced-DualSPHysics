  ...
  ,const tuint3 &celfmin,const tuint3 &celfmax,tuint3 &celmin,tuint3 &celmax)const
{
  ...
  celmin=TUint3(min(min(celbmin.x,celfmin.x),(celfmin.x>=scelldiv? celfmin.x-scelldiv: 0))
               ,min(min(celbmin.y,celfmin.y),(celfmin.y>=scelldiv? celfmin.y-scelldiv: 0))
               ,min(min(celbmin.z,celfmin.z),(celfmin.z>=scelldiv? celfmin.z-scelldiv: 0)));
  celmax=TUint3(max(max(celbmax.x,celfmax.x),celfmax.x+scelldiv)
               ,max(max(celbmax.y,celfmax.y),celfmax.y+scelldiv)
               ,max(max(celbmax.z,celfmax.z),celfmax.z+scelldiv));
