...
typedef struct StrInterParmsg{
  ...
  TpDivClean tdiv; TpVolConserve tvol; TpOps tops;
  //-Execution values.
  float viscob,viscof;
  ...
  float3 *normalvec;
  unsigned *pardet;
  float *pressvcs; float3 *shiftvcs; float *rhobar;
  float *pressvem; float3 *acevem; float *psi; float* apsi; float* veldiv;
  ...
  StrInterParmsg(
    ...
    ,float4 *shiftposfs_
    ,tmatrix3f* corrmat_, float3* normalvec_, unsigned* pardet_
    ,float* pressvcs_, float3* shiftvcs_, float* rhobar_
    ,float* pressvem_, float3* acevem_, float* psi_, float* apsi_, float* veldiv_
    ,cudaStream_t stm_
    ,StKerInfo *kerinfo_)
  {
    //-Configuration options.
    ...
    tdiv=tdiv_; tvol=tvol_; tops=tops_;
    //-Execution values.
    ...
    corrmat=corrmat_;normalvec=normalvec_;pardet=pardet_;
    pressvcs=pressvcs_;shiftvcs=shiftvcs_; rhobar=rhobar_;
    pressvem=pressvem_;acevem = acevem_; psi=psi_; apsi=apsi_;veldiv=veldiv_;
    //-Other values and objects.
    stm=stm_;
    kerinfo=kerinfo_;
  }
