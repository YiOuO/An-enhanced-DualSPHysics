...
__device__ float ComputeRhoMonaghanCte(float press) {
	//return(CTE.cteb * (pow(rhop * CTE.ovrhopzero, CTE.gamma) - 1.0f));
	return(CTE.rhopzero * pow(double(press) / double(CTE.cteb) + 1.0f, 1.0f / double(CTE.gamma)));
}

__device__ float ComputeRhoCte(float press) {
	return(ComputeRhoMonaghanCte(press));
}
#endif
//------------------------------------------------------------------------------
/// Returns pressure starting from density using equation of state 
