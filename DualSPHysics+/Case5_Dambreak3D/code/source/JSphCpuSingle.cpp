...
void JSphCpuSingle::SaveData(){
  ...
  tfloat3 *vel=NULL;
  float *rhop=NULL;
  unsigned *pardet=NULL;
  tfloat3 *normalvec=NULL;
  float *rhobar=NULL;
  float *veldiv=NULL;
  if(save){
  ...
  JDataArrays arrays;
  AddBasicArrays(arrays,npsave,pos,idp,vel,rhop,pardet,normalvec,rhobar,veldiv);
  ...
  ArraysCpu->Free(vel);
  ArraysCpu->Free(rhop);
  ArraysCpu->Free(pardet);
  ArraysCpu->Free(normalvec);
