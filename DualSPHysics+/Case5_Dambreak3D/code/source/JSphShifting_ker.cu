...
/// Calcula Shifting final para posicion de particulas.
//------------------------------------------------------------------------------
__global__ void KerRunShiftingOPS(unsigned n, unsigned pini, double dt
    , double coefumagn, float maxdist, const float4* velrhop, float4* shiftposfs)
{
    const unsigned p = blockIdx.x * blockDim.x + threadIdx.x;
    if (p < n) {
        const unsigned p1 = p + pini;
        float4 rs = shiftposfs[p1];
        if (rs.x != FLT_MAX) {
            const double shiftdistx = double(double(rs.x) * coefumagn);
            const double shiftdisty = double(double(rs.y) * coefumagn);
            const double shiftdistz = double(double(rs.z) * coefumagn);
...
/// Calcula Shifting final para posicion de particulas.
//==============================================================================
void RunShiftingOPS(unsigned n, unsigned pini, double dt
    , double coefumagn, float maxdist, const float4* velrhop
    , float4* shiftposfs, cudaStream_t stm)
{
    if (n) {
        const dim3 sgrid = GetSimpleGridSize(n, SPHBSIZE);
        KerRunShiftingOPS << <sgrid, SPHBSIZE, 0, stm >> > (n, pini, dt, coefumagn, maxdist, velrhop, shiftposfs);
    }
}
