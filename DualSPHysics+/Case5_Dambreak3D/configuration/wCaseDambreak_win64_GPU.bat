...
:postprocessing
@REM rem Executes PartVTK to create VTK files with particles.
set dirout2=%dirout%\particles
%partvtk% -dirin %diroutdata% -savevtk %dirout2%/PartFluid -onlytype:-all,+fluid -vars:+partype,+normalvec,+press,+rho_rho0,+veldiv
if not "%ERRORLEVEL%" == "0" goto fail


set dirout2=%dirout%\particles
%partvtk% -dirin %diroutdata% -savevtk %dirout2%/PartBound -onlytype:-all,+bound -vars:+partype,+normalvec,+press
if not "%ERRORLEVEL%" == "0" goto fail
...
%measuretool% -dirin %diroutdata% -points elevation.txt -onlytype:-all,+fluid -elevation:0.5 -savecsv %dirout2%/_elevation  
if not "%ERRORLEVEL%" == "0" goto fail

%measuretool% -dirin %diroutdata% -points pressure.txt -onlytype:+all,+fluid,+bound -vars:-all,+press -kclimit:0.5 -savecsv %dirout2%/_pressure 
if not "%ERRORLEVEL%" == "0" goto fail
