...
${partvtk} -dirin ${diroutdata} -savevtk ${dirout2}/PartBound -onlytype:-all,+bound -vars:+press,+VelDiv,NormalVec,ParType
if [ $? -ne 0 ] ; then fail; fi

${partvtk} -dirin ${diroutdata} -savevtk ${dirout2}/PartFluid -onlytype:-all,+fluid -vars:+press,+VelDiv,NormalVec,ParType,+Rho_Rho0
if [ $? -ne 0 ] ; then fail; fi
