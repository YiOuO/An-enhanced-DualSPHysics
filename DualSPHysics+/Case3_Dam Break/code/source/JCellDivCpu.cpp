...
void JCellDivCpu::AllocMemoryNp(ullong np){
  ...
  try{
    ...
    SetMemoryVSort(new byte[sizeof(tmatrix2d)*SizeNp]);  MemAllocNp+=sizeof(tdouble3)*SizeNp;
  }
...
/// Reordena datos de todas las particulas (para tipo unsigned).
//==============================================================================
void JCellDivCpu::SortArray(tmatrix2d* vec) {
    const int n = int(Nptot);
    const int ini = (DivideFull ? 0 : int(NpbFinal));
    #ifdef OMP_USE
        #pragma omp parallel for schedule (static) if(n>OMP_LIMIT_COMPUTELIGHT)
    #endif
    for (int p = ini; p < n; p++)VSortMatrix2d[p] = vec[SortPart[p]];
    memcpy(vec + ini, VSortMatrix2d + ini, sizeof(tmatrix2d) * (n - ini));
}
