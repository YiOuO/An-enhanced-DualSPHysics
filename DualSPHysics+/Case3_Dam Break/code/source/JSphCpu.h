...
typedef struct {
    ...
    const float* press;
    const unsigned* pardet;
    const tmatrix2d* corrmat;
    float* pressvem;
    tdouble3* acevem;
    float* pressvcs;
    tdouble3* shiftvcs;
...
inline stvemvcsparmsc StVEMVCSparm(bool vem, bool vcs, unsigned np, unsigned npb, unsigned npbok
    , StDivDataCpu divdata, const unsigned* dcell
    , const tdouble3* pos, const tfloat4* velrhop
    , float* press, const unsigned* pardet, const tmatrix2d* corrmat
    , float* pressvem, tdouble3* acevem, float* pressvcs, tdouble3* shiftvcs)
{
    stvemvcsparmsc d = {vem,vcs,np,npb,npbok,(np - npb),divdata,dcell
      ,pos,velrhop,press,pardet,corrmat,pressvem,acevem,pressvcs,shiftvcs
    };
    return(d);
}
...
inline stvcsparmsc StVcsparm(unsigned np, unsigned npb, unsigned npbok
    , StDivDataCpu divdata, const unsigned* dcell
    , const tdouble3* pos, const tfloat4* velrhop
    , float* press,const unsigned*pardet, const tmatrix2d *corrmat,float* pressvcs,tdouble3 *shiftvcs)
{
    stvcsparmsc d = { np,npb,npbok,(np - npb)
      ,divdata,dcell
      ,pos,velrhop,press,pardet,corrmat,pressvcs,shiftvcs
    };
    return(d);
}
...
inline stvemparmsc StVemparm(unsigned np, unsigned npb, unsigned npbok
    , StDivDataCpu divdata, const unsigned* dcell
    , const tdouble3* pos, const tfloat4* velrhop
    , float* pressvem)
{
    stvemparmsc d = { np,npb,npbok,(np - npb)
      ,divdata,dcell
      ,pos,velrhop,pressvem
    };
    return(d);
}
...
typedef struct {
    ...
    const tdouble3* pos;
    const tfloat4* velrhop;
    const float* pressvem;
    const unsigned* pardet;
    tdouble3* acevem;
}stvemcorrparmsc;

inline stvemcorrparmsc StVemCorrparm(unsigned np, unsigned npb, unsigned npbok
    , StDivDataCpu divdata, const unsigned* dcell
    , const tdouble3* pos, const tfloat4* velrhop
    , const float* pressvem, const unsigned* pardet,tdouble3* acevem)
{
    stvemcorrparmsc d = { np,npb,npbok,(np - npb)
      ,divdata,dcell
      ,pos,velrhop,pressvem,pardet,acevem
    };
    return(d);
}
...
inline stinterparmsc StInterparmsc(unsigned np,unsigned npb,unsigned npbok
  ...
  ,const float *press,const tfloat3 *dengradcorr,float* ar,tfloat3 *ace,float *delta
  ,TpShifting shiftmode,tfloat4 *shiftposfs
  ,tsymatrix3f *spstau,tsymatrix3f *spsgradvel,tmatrix2d* corrmat,unsigned* pardet
  ,tfloat3* parnormal,float* psi,float* apsi
)
{
  stinterparmsc d={np,npb,npbok,(np-npb)
    ...
    ,spstau,spsgradvel,corrmat,pardet,parnormal,psi,apsi
  };
  return(d);
}

///Collects parameters for particle interaction on CPU for Velocity Div.
inline stinterparmsc StInterparmsc(unsigned np, unsigned npb, unsigned npbok,
...
protected:
  ...
  tmatrix2d* CorrMatc; //-Correct Matrix 2x2 for 2d problem
  unsigned* ParDet;    //-Particle Detection 0-splash 1-free surface 2-free surface vicinity 3-inner 4-wall vicinity
  tfloat3* NormalVec;  //-Normal Vector 

  bool VemAdded = false;
  bool VcsAdded = false;
  bool HpdcAdded = false;
  //---------------------------
  float* PressVemc; //-Vem Press
  float* PressVcsc; //-Vcs Press
  float* VelDivc;   //-Output velocity divergence
  tdouble3* ShiftVcsC; //-Shifing related to VCS pressure
  tdouble3* AceVemc;   //-Acceleration related to VEM pressure
  ...
  float* viscoArray;   //-1/max(pi_ij)(for OPS)
  float* RhoOver;      //-Rhoi/rho0
  float c_ad=0.1, c_ac=0.5;//-Coef in e.q.(21) in Khayyer et al. AMM2023 for OPS
  double rhoref;       //-Reference kernel summation based density(for VCS)
  
  float* Psic;//-HPDC \Psi
  float* PsiPrec;//-PsiPre(Symplectic step)
  float* APsi;//-dPsi/dt
  ...
  template<TpKernel tker, TpFtMode ftmode> void InteractionForcesBoundDummy
  (unsigned n, unsigned pini, StDivDataCpu divdata, const unsigned* dcell
      , const tdouble3* pos, tfloat4* velrhop, const typecode* code, const unsigned* id
      , float* press, float& viscdt, float* ar, float* psi)const;
  ...
  template<TpKernel tker,TpFtMode ftmode,TpVisco tvisco,TpDensity tdensity,bool shift> 
    ...
    ,TpShifting shiftmode,tfloat4 *shiftposfs , const tmatrix2d* corrmat, const unsigned* pardet
  , const tfloat3* normalvec,const float* psi,float *apsi)const;
...
public:
    float ek0 = 0, ep0 = 0, et0=0;            // initial kinetic energy 
    void CalDiv(const stinterparmsc& t)const;

    template<TpKernel tker> void CalDivFin(unsigned n, unsigned pinit, StDivDataCpu divdata, const unsigned* dcell,
        const tdouble3* pos, const tfloat4* velrhop, const typecode* code, const unsigned* idp)const;

    void GetRhoRef(const stinterparmsc& t);

    template<TpKernel tker> void GetRhoFin(unsigned n, unsigned pinit, StDivDataCpu divdata, const unsigned* dcell,
        const tdouble3* pos, const tfloat4* velrhop, const typecode* code, const unsigned* idp);

    void GetCorrectMatrix(unsigned n, unsigned pinit, StDivDataCpu divdata, const unsigned* dcell
        , const tdouble3* pos, const tfloat4* velrhop, tmatrix2d* corrmat);
    void GetCorrectMatrixBound(unsigned n, unsigned pinit, StDivDataCpu divdata, const unsigned* dcell
        , const tdouble3* pos, const tfloat4* velrhop, tmatrix2d* corrmat);
    
    void GetNormals(unsigned n, unsigned pinit, tmatrix2d* corrmat, tfloat3* parnormal
        , const tfloat4* velrhop, const tdouble3* pos
        , StDivDataCpu divdata, const unsigned* dcell);
    void GetNormalsBound(unsigned n, unsigned pinit, tmatrix2d* corrmat, tfloat3* parnormal
        , const tfloat4* velrhop, const tdouble3* pos
        , StDivDataCpu divdata, const unsigned* dcell);

    void FreeSurfaceDetction(unsigned n, unsigned pinit, tmatrix2d* corrmat, unsigned* pardet
        , tfloat3* parnormal, const tfloat4* velrhop, const tdouble3* pos
        , StDivDataCpu divdata, const unsigned* dcell);

    void VcsStep(const stvcsparmsc& t, double dt)const;
    template<TpKernel tker> void Vcs_ct2(const stvcsparmsc& t, double dt)const;

    template<TpKernel tker> void RunVcsPress
    (unsigned n, unsigned pini, StDivDataCpu divdata, const unsigned* dcell
        , const tdouble3* pos, const tfloat4* velrhop, const float* press
        , float* pressvcs, const unsigned* pardet, double dt
    )const;

    template<TpKernel tker> void RunVcsShift
    (unsigned n, unsigned pini, StDivDataCpu divdata, const unsigned* dcell
        , const tdouble3* pos, const tfloat4* velrhop
        , const float* pressvcs, const double dt
        , tdouble3* shiftvcs, const tmatrix2d* corrmat
    )const;  

    template<TpKernel tker,bool shift>void ShiftFluid(unsigned n,unsigned pini
    ...
    void RunVemPress_t1(const stvemparmsc& t, double dt)const;

    void RunVEMVCS_StepI(const stvemvcsparmsc& t, double dt)const;
    void RunVEMVCS_StepII(const stvemvcsparmsc& t, double dt)const;

    template<TpKernel tker> void RunVEMVCS_StepI_Fin(const bool vem, const bool vcs, unsigned n, unsigned pinit, StDivDataCpu divdata, const unsigned* dcell,
        const tdouble3* pos, const tfloat4* velrhop, const unsigned* pardet, float* pressvem, float* pressvcs, double dt) const;

    template<TpKernel tker> void RunVEMVCS_StepII_Fin(const bool vem, const bool vcs, unsigned n, unsigned pinit, StDivDataCpu divdata, const unsigned* dcell,
        const tdouble3* pos, const tfloat4* velrhop, const tmatrix2d* corrmat, const unsigned* pardet, float* pressvem, tdouble3* acevem ,float* pressvcs, tdouble3* shiftvcs, double dt) const;

    template<TpKernel tker> void RunVemPress_Fin(unsigned n, unsigned pinit, StDivDataCpu divdata, const unsigned* dcell,
        const tdouble3* pos, const tfloat4* velrhop, float* pressvem, double dt) const;

    void VemPressInteraction_t1(const stvemcorrparmsc& t)const;

    template<TpKernel tker> void VemPressInteraction_Fin(unsigned n, unsigned pinit, StDivDataCpu divdata, const unsigned* dcell,
