...
/// Returns pressure starting from density using default equation of state.
//------------------------------------------------------------------------------
inline double ComputePress(float rhop, const StCteSph& csp, bool loworder) {
	return(csp.cs0 * csp.cs0 * double((double(rhop) - double(csp.rhopzero))));
}


/// Returns pressure starting from density using equation of state 
/// based on [Monaghan, 1994].
inline float ComputeRhoMonaghan(float press, float rhop0, float b, float gamma) {
	// return(b*(pow(rhop/rhop0,gamma)-1.0f));
	float cc = rhop0 * pow(press / b + 1.0f, 1.0 / gamma);
	return(rhop0 * pow(press / b + 1.0f, 1.0 / gamma));
}
//==============================================================================
inline float ComputeRhoMonaghan(float press, const StCteSph& csp) {
	return(ComputeRhoMonaghan(press, csp.rhopzero, csp.cteb, csp.gamma));
}


inline float ComputeRho(float press, const StCteSph& csp) {
	return(ComputeRhoMonaghan(press, csp));
}

inline float ComputeRho(float press, const StCteSph& csp, bool loworder) {
	return(press / csp.cs0 / csp.cs0 + csp.rhopzero);
