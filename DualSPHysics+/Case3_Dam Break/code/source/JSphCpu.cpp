...
void JSphCpu::AllocCpuMemoryParticles(unsigned np,float over){
    ...
    #endif
    ArraysCpu->AddArrayCount(JArraysCpu::SIZE_4B, 10);  //-idp,ar,viscdt,dcell,prrhop,pardet,veldiv
    if (DDTArray)ArraysCpu->AddArrayCount(JArraysCpu::SIZE_4B, 1);  //-delta

    ArraysCpu->AddArrayCount(JArraysCpu::SIZE_12B, 3); //-ace,parnormal
    ArraysCpu->AddArrayCount(JArraysCpu::SIZE_16B, 2); //-velrhop,poscell
    ArraysCpu->AddArrayCount(JArraysCpu::SIZE_24B, 4); //-pos,shiftvcs,acevem
    ArraysCpu->AddArrayCount(JArraysCpu::SIZE_32B, 1); //-corrmat
    ArraysCpu->AddArrayCount(JArraysCpu::SIZE_4B, 2);  //-viscoarray,rhoover
    if (TStep == STEP_Verlet) {
        ...
        ArraysCpu->AddArrayCount(JArraysCpu::SIZE_4B, 1); //-psipre
    }
    ...
    if (TDivClean == DIV_VEM) {
        ArraysCpu->AddArrayCount(JArraysCpu::SIZE_4B, 1); //-VEM Pressure
        ArraysCpu->AddArrayCount(JArraysCpu::SIZE_24B, 1);//-VEM Acceleration
    }
    else if (TDivClean == DIV_VEM_HPDC) {
        ArraysCpu->AddArrayCount(JArraysCpu::SIZE_4B, 4); //-Psi,Psipre,APsi,VEM Pressure
        ArraysCpu->AddArrayCount(JArraysCpu::SIZE_24B, 1);//-VEM Acceleration
    }
    if (TVolConserve == VOL_VCS) {
        ArraysCpu->AddArrayCount(JArraysCpu::SIZE_24B, 1);//-Shifting(VCS)
        ArraysCpu->AddArrayCount(JArraysCpu::SIZE_4B, 1);//-VCS Press
    }
...
void JSphCpu::ReserveBasicArraysCpu(){
    ...
    Posc = ArraysCpu->ReserveDouble3();
    Velrhopc = ArraysCpu->ReserveFloat4();
    CorrMatc = ArraysCpu->ReserveMatrix2d();
    ParDet = ArraysCpu->ReserveUint();
    NormalVec = ArraysCpu->ReserveFloat3();
    VelDivc = ArraysCpu->ReserveFloat();
    viscoArray = ArraysCpu->ReserveFloat();
    ...
    if (TDivClean == DIV_VEM) {
        PressVemc = ArraysCpu->ReserveFloat();
        AceVemc = ArraysCpu->ReserveDouble3();
    }
    else if (TDivClean == DIV_VEM_HPDC) {
        PressVemc = ArraysCpu->ReserveFloat();
        AceVemc = ArraysCpu->ReserveDouble3();
        Psic = ArraysCpu->ReserveFloat();
    }
    if (TVolConserve == VOL_VCS) {
        ShiftVcsC = ArraysCpu->ReserveDouble3();
        PressVcsc = ArraysCpu->ReserveFloat();
    }
}
...
unsigned JSphCpu::GetParticlesData(unsigned n,unsigned pini,bool onlynormal
  ,unsigned *idp,tdouble3 *pos,tfloat3 *vel,float *rhop, unsigned* pardetction
  , float* veldiv,float *rhoover,tfloat3* parnormal,typecode *code)
{
  unsigned num=n;
  ...
  if (pardetction) memcpy(pardetction, ParDet + pini, sizeof(unsigned) * n);
  if (veldiv) memcpy(veldiv, VelDivc + pini, sizeof(float) * n);
  if (rhoover) memcpy(rhoover, RhoOver + pini, sizeof(float) * n);
  if (parnormal)memcpy(parnormal, NormalVec + pini, sizeof(tfloat3) * n);
  if(vel && rhop){
...
void JSphCpu::InitRunCpu(){
  ...
  VemAdded = (TDivClean != DIV_NONE ? true : false);
  VcsAdded = (TVolConserve == VOL_VCS ? true : false);
  HpdcAdded = (TDivClean == DIV_VEM_HPDC ? true : false);
  ...
  if (VcsAdded)memset(PressVcsc, 0, sizeof(float) * Np);
  if (HpdcAdded)memset(Psic, 0, sizeof(float) * Np);
}
...
void JSphCpu::PreInteractionVars_Forces(unsigned np,unsigned npb){
  ...
  if(APsi)memset(APsi, 0, sizeof(float) * np);                                //Apsi[]=0
  ...
  if(ShiftPosfsc)memset(ShiftPosfsc, 0, sizeof(tfloat4) * np);
...
void JSphCpu::PreInteraction_Forces(){
  ...
  if (HpdcAdded)APsi = ArraysCpu->ReserveFloat();
...
void JSphCpu::PosInteraction_Forces(){
  ...
  if (HpdcAdded) { ArraysCpu->Free(APsi); APsi = NULL; }
}
/// Perform interaction between particles. Bound-Fluid/Float
/// Realiza interaccion entre particulas. Bound-Fluid/Float
//==============================================================================
template<TpKernel tker, TpFtMode ftmode> void JSphCpu::InteractionForcesBoundDummy
(unsigned n, unsigned pinit, StDivDataCpu divdata, const unsigned* dcell
    ...
    , float* press
    , float& viscdt, float* ar, float* psi)const
{
    ...
    for (int p1 = int(pinit); p1 < pfin; p1++) {
        ...
        //{
                ...
                for (unsigned p2 = pif.x; p2 < pif.y; p2++) {
                    ...
                    if (rr2 <= KernelSize2 && rr2 >= ALMOSTZERO) {
                        ...
                        if (compute) {
                            ...
                            // press boundary
                            pressP1 += (press[p2] + (Gravity.x * drx + Gravity.y * dry + Gravity.z * drz) * velrhop2.w) * wab;
            ...
            pressP1 = (kernelSum != 0)? pressP1 / kernelSum:0;

            press[p1] = pressP1;
            velrhop[p1].w = fsph::ComputeRho(press[p1], CSP);//-Pressure to density
            //psi[p1] = psiP1;
            if (HpdcAdded)psi[p1] = 0.0; //-Dirichlet Boundary conditions for PSI is recommended
  ...
  ,float &viscdt,float *ar)const
{
  ...
  for(int p1=int(pinit);p1<pfin;p1++){
    ...
    const bool rsymp1=(Symmetry && posp1.y<=KernelSize); //<vs_syymmetry>
    const tfloat4 velrhop1=velrhop[p1];
    float vel = sqrt(velrhop1.x * velrhop1.x + velrhop1.y * velrhop1.y + velrhop1.z * velrhop1.z);
    float t1 = KernelH / (vel != 0 ? vel : FLT_MIN);
    //-Search for neighbours in adjacent cells.
    const StNgSearch ngs=nsearch::Init(dcell[p1],false,divdata);
    for(int z=ngs.zini;z<ngs.zfin;z++)for(int y=ngs.yini;y<ngs.yfin;y++){
      const tuint2 pif=nsearch::ParticleRange(y,z,ngs,divdata);
        ...
        if(rr2<=KernelSize2 && rr2>=ALMOSTZERO){
          ...
          if(compute){
            ...
            {//-Viscosity.
              const float dot=drx*dvx + dry*dvy + drz*dvz;
              const float dot_rr2=dot/(rr2+Eta2);
              viscoArray[p1] = min(1/(fabs(dot_rr2)!=0?fabs(dot_rr2):FLT_MIN), viscoArray[p1]);
              viscoArray[p1] = min(t1, viscoArray[p1]);
              visc=max(dot_rr2,visc);
            }
          }
          rsym=(rsymp1 && !rsym && float(posp1.y-dry)<=KernelSize); //<vs_syymmetry>
...
template<TpKernel tker,TpFtMode ftmode,TpVisco tvisco,TpDensity tdensity,bool shift> 
  ...
  ,TpShifting shiftmode,tfloat4 *shiftposfs,const tmatrix2d* corrmat
  , const unsigned* pardet, const tfloat3* normalvec
  ,const float* psi,float *apsi)const
{
  ...
  #endif
  for(int p1=int(pinit);p1<pfin;p1++){
    float visc=0,arp1=0,deltap1=0,apsip1=0;
    ...
    const bool rsymp1=(Symmetry && posp1.y<=KernelSize); //<vs_syymmetry>
    float vel = sqrt(velp1.x * velp1.x + velp1.y * velp1.y + velp1.z * velp1.z);
    float t1 = KernelH / (vel != 0 ? vel : 0.0001);
    float psip1 = 0; if (HpdcAdded) psip1 = psi[p1];
    //-Search for neighbours in adjacent cells.
    ...
    for(int z=ngs.zini;z<ngs.zfin;z++)for(int y=ngs.yini;y<ngs.yfin;y++){
      ...
      for(unsigned p2=pif.x;p2<pif.y;p2++){
        ...
        if(rr2<=KernelSize2 && rr2>=ALMOSTZERO){
          ...
          //=====================================================================
          //=================Riemann Solver For Momentum equation================
          //=====================================================================
          // one-side riemann sph problem
          tdouble3 eij = TDouble3(-drx, 0, -drz) / double(sqrt(rr2));
          const double ul = boundp2 ? (-normalvec[p2].x * velp1.x + -normalvec[p2].z * velp1.z):(velp1.x * eij.x + velp1.z * eij.z);
          const double ur = boundp2 ? -ul-2.0*(normalvec[p2].x * velrhop2.x + normalvec[p2].z * velrhop2.z):(velrhop2.x * eij.x + velrhop2.z * eij.z);//Zhang JCP2017
          const double pl = pressp1, pr = boundp2? (pressp1+ velrhop[p1].w*(Gravity.x*-drx + Gravity.z*-drz)):press[p2]; //One sided Riemann
          velrhop2.w = boundp2 ? fsph::ComputeRho(pr, CSP) : velrhop2.w;
          const double cl = Cs0, cr = Cs0;
          const double rhol = rhopp1, rhor = velrhop2.w;
          const double beta0 = Beta;
          if (compute) {
              //-Velocity derivative (Momentum equation).
              if (pressp1 <= 0 && pardet[p1] == 3) {
                  const double phi0 = beta0 * KernelH / sqrt(rr2) * min(max(ul - ur, 0), Cs0);
                  const double Pstar = 0.5 * (press[p2] - pressp1) + 0.5 * phi0 * (rhopp1 + velrhop2.w) / 2.0 * (ul - ur);
                  const double p_vpm = -2.0 * massp2 * Pstar / (rhopp1 * velrhop2.w);
                  acep1.x += p_vpm * frx; acep1.y += p_vpm * fry; acep1.z += p_vpm * frz;
              }
              else
              {
                  const double phi0 = beta0 * KernelH / sqrt(rr2) * min(max(ul - ur, 0), Cs0);
                  const double Pstar = 0.5 * (pressp1 + press[p2]) + 0.5 * phi0 * (rhopp1 + velrhop2.w) / 2.0 * (ul - ur);
                  const double p_vpm = -2.0 * massp2 * Pstar / (rhopp1 * velrhop2.w);
                  acep1.x += p_vpm * frx; acep1.y += p_vpm * fry; acep1.z += p_vpm * frz;
              }

              //-Psi in momentum equation
              if (HpdcAdded) {
          ...
          //-Psi derivative (dPsi/dt).
          if (HpdcAdded) {
              const float dvx = velp1.x - velrhop2.x, dvy = velp1.y - velrhop2.y, dvz = velp1.z - velrhop2.z;
              apsip1 += Cs0 * Cs0 * massp2 * (dvx * frx + dvy * fry + dvz * frz) / velrhop2.w;
          ...
          if(compute){
            ...
            const float dot_rr2=dot/(rr2+Eta2);
            visc=max(dot_rr2,visc);
            viscoArray[p1] = min(1 / (fabs(dot_rr2) != 0 ? fabs(dot_rr2): 0.0001), viscoArray[p1]);
            viscoArray[p1] = min(t1, viscoArray[p1]);
    ...
    if (HpdcAdded && (!boundp2)) {
        const float _tau = Cs0 * PI / (2.0f * KernelH);
        apsip1 += -_tau * psip1;
    }
    //-Sum results together. | Almacena resultados.
  ...
  void JSphCpu::Interaction_ForcesCpuT(const stinterparmsc &t,StInterResultc &res)const
{
  ...
  if (t.npbok) {
      ...
      InteractionForcesBoundDummy<tker, ftmode>(t.npb, 0, t.divdata, t.dcell
          , t.pos, Velrhopc, t.code, t.idp, Pressc, viscdt, t.ar, t.psi);
  }
  ...
  if(t.npf){
    ...
    InteractionForcesFluid<tker,ftmode,tvisco,tdensity,shift> (t.npf,t.npb,false,Visco                 
      ...
      ,viscdt,t.ar,t.ace,t.delta,t.shiftmode,t.shiftposfs,t.corrmat,t.pardet,t.parnormal
      ,t.psi,t.apsi);
    ...
    InteractionForcesFluid<tker,ftmode,tvisco,tdensity,shift> (t.npf,t.npb,true ,Visco*ViscoBoundFactor
      ...
      ,t.psi, t.apsi);
    //-Get Shifting Value
    if(Shifting)ShiftFluid<tker, shift>(t.npf, t.npb
        , t.divdata, t.dcell, t.pos, t.velrhop, t.shiftposfs, t.corrmat, t.pardet, t.parnormal);
...
double JSphCpu::DtVariable(bool final){
  ...
  //double dt = 0.25*double(KernelH / (Cs0 + abs(VelMax)));//A simple way
...
void JSphCpu::RunShifting(double dt){
  Timersc->TmStart(TMC_SuShifting);
   Shifting->RunCpuOps(Np - Npb, Npb, dt, Velrhopc, ShiftPosfsc,viscoArray,c_ad);
...
void JSphCpu::GetCorrectMatrix
(unsigned n, unsigned pinit, StDivDataCpu divdata, const unsigned* dcell
    , const tdouble3* pos, const tfloat4* velrhop, tmatrix2d* corrmat
)
{
    const int pfin = int(pinit + n);
    #ifdef OMP_USE
        #pragma omp parallel for schedule (guided)
    #endif
    for (int p1 = pinit; p1 < pfin; p1++) {
        const tdouble3 posp1 = pos[p1];
        tmatrix2d coma = TMatrix2d(0);
        // find neiboor fluid particle
        StNgSearch ngs = nsearch::Init(dcell[p1], false, divdata);
        for (int z = ngs.zini; z < ngs.zfin; z++)for (int y = ngs.yini; y < ngs.yfin; y++) {
            const tuint2 pif = nsearch::ParticleRange(y, z, ngs, divdata);
            for (unsigned p2 = pif.x; p2 < pif.y; p2++) {
                const float drx = float(posp1.x - pos[p2].x);
                ...
                if (rr2 <= KernelSize2 && rr2 >= ALMOSTZERO) {
                    //-Computes kernel.
                    const float fac = fsph::GetKernel_Fac<KERNEL_Wendland>(CSP, rr2);
                    const float frx = fac * drx, fry = fac * dry, frz = fac * drz; //-Gradients.
                    float massp2 = MassFluid;
                    float vj = massp2 / velrhop[p2].w;
                    coma.a11 += -drx * frx * vj;
                    coma.a21 += -drz * frx * vj;
                    coma.a12 += -drx * frz * vj;
                    coma.a22 += -drz * frz * vj;
        ...
        // find neiboor bound particle
        ngs = nsearch::Init(dcell[p1], true, divdata);
        for (int z = ngs.zini; z < ngs.zfin; z++)for (int y = ngs.yini; y < ngs.yfin; y++) {
            const tuint2 pif = nsearch::ParticleRange(y, z, ngs, divdata);
            for (unsigned p2 = pif.x; p2 < pif.y; p2++) {
                const float drx = float(posp1.x - pos[p2].x);
        ...
        const double determ = fmath::Determinant2x2(coma);
        const tmatrix2d invcoma = fmath::InverseMatrix2x2(coma, determ);
        corrmat[p1] = invcoma;
    }

}

void JSphCpu::GetCorrectMatrixBound
(unsigned n, unsigned pinit, StDivDataCpu divdata, const unsigned* dcell
    , const tdouble3* pos, const tfloat4* velrhop, tmatrix2d* corrmat)
{
    const int pfin = int(pinit + n);
    ...
    for (int p1 = pinit; p1 < pfin; p1++) {
        ...
        tmatrix2d coma = TMatrix2d(0);
        // find neiboor fluid particle
        StNgSearch ngs = nsearch::Init(dcell[p1], true, divdata);//consider bound particles only
        for (int z = ngs.zini; z < ngs.zfin; z++)for (int y = ngs.yini; y < ngs.yfin; y++) {
            const tuint2 pif = nsearch::ParticleRange(y, z, ngs, divdata);
            for (unsigned p2 = pif.x; p2 < pif.y; p2++) {
                const float drx = float(posp1.x - pos[p2].x);
...
void JSphCpu::GetNormals(unsigned n, unsigned pinit, tmatrix2d* corrmat, tfloat3* parnormal
    , const tfloat4* velrhop, const tdouble3* pos
    , StDivDataCpu divdata, const unsigned* dcell) {
    const int pfin = int(pinit + n);
    float* Ci = new float[pfin]();
    // Get particle concentration C
    #ifdef OMP_USE
        #pragma omp parallel for schedule (guided)
    #endif
    for (int p1 = pinit; p1 < pfin; p1++) {
        ...
        const float rhopp1 = velrhop[p1].w;
        float ci = 0.0;
        //-Search for neighbour fluid particles in adjacent cells.
        bool boundp2 = false;
        StNgSearch ngs = nsearch::Init(dcell[p1], boundp2, divdata);
        for (int z = ngs.zini; z < ngs.zfin; z++)for (int y = ngs.yini; y < ngs.yfin; y++) {
            const tuint2 pif = nsearch::ParticleRange(y, z, ngs, divdata);
            bool rsym = false;
            for (unsigned p2 = pif.x; p2 < pif.y; p2++) {
                ...
                const float rr2 = drx * drx + dry * dry + drz * drz;
                if (rr2 <= KernelSize2 && rr2 >= ALMOSTZERO) {
                    const float wab = fsph::GetKernel_Wab<KERNEL_Wendland>(CSP, rr2);
                    //===== Get mass of particle p2 ===== 
                    float massp2 = (boundp2 ? MassBound : MassFluid);
                    ci += wab * massp2 / velrhop[p2].w;
                }
            }
        }
        //-Search for neighbour bound particles in adjacent cells.
        boundp2 = true;
        ngs = nsearch::Init(dcell[p1], boundp2, divdata);
        for (int z = ngs.zini; z < ngs.zfin; z++)for (int y = ngs.yini; y < ngs.yfin; y++) {
            const tuint2 pif = nsearch::ParticleRange(y, z, ngs, divdata);
            bool rsym = false;
            for (unsigned p2 = pif.x; p2 < pif.y; p2++) {
        ...
        Ci[p1] = ci + fsph::GetKernel_Wab<KERNEL_Wendland>(CSP, 0) * MassFluid / rhopp1;
    }
    ...
    for (int p1 = pinit; p1 < pfin; p1++) {
        ...
        for (int z = ngs.zini; z < ngs.zfin; z++)for (int y = ngs.yini; y < ngs.yfin; y++) {
            ...
            for (unsigned p2 = pif.x; p2 < pif.y; p2++) {
                ...
                if (rr2 <= KernelSize2 && rr2 >= ALMOSTZERO) {
                    //-Computes kernel.        
                    const float fac = fsph::GetKernel_Fac<KERNEL_Wendland>(CSP, rr2);
                    const float frx = fac * drx, fry = fac * dry, frz = fac * drz; //-Gradient

                    //===== Get mass of particle p2 ===== 
                    float massp2 = (boundp2 ? MassBound : MassFluid);
                    float volp2 = massp2 / velrhop[p2].w;
                    nablaC.x += (Ci[p2] - Ci[p1]) * (corrmat[p1].a11 * frx + corrmat[p1].a12 * frz) * volp2;
                    nablaC.y += 0;
                    nablaC.z += (Ci[p2] - Ci[p1]) * (corrmat[p1].a21 * frx + corrmat[p1].a22 * frz) * volp2;
                }
            }
        }
        //-Search for neighbour bound particles in adjacent cells.
        ...
        float nablaCNorm = TFloat3Norm(nablaC);
        parnormal[p1] = TFloat3(-nablaC.x / nablaCNorm, -nablaC.y / nablaCNorm, -nablaC.z / nablaCNorm); //nomarl vec of particle p1
    }
    delete[] Ci;

}

void JSphCpu::GetNormalsBound(unsigned n, unsigned pinit, tmatrix2d* corrmat, tfloat3* parnormal
    , const tfloat4* velrhop, const tdouble3* pos
    , StDivDataCpu divdata, const unsigned* dcell) {
    const int pfin = int(pinit + n);
    float* Ci = new float[pfin]();
    ...
    for (int p1 = pinit; p1 < pfin; p1++) {
        ...
        Ci[p1] = ci + fsph::GetKernel_Wab<KERNEL_Wendland>(CSP, 0) * Dp * Dp;
    }
...
void JSphCpu::FreeSurfaceDetction(unsigned n, unsigned pinit, tmatrix2d* corrmat, unsigned* pardet
    , tfloat3* parnormal, const tfloat4* velrhop, const tdouble3* pos
    , StDivDataCpu divdata, const unsigned* dcell) {
    GetNormalsBound(pinit, 0, corrmat, parnormal, velrhop, pos, divdata, dcell);
    GetNormals(n, pinit, corrmat, parnormal, velrhop, pos, divdata, dcell);
    const int pfin = int(pinit + n);
    float* posdiv = new float[pfin]();//div of postion
    unsigned* Ni = new unsigned[pfin]();//neiboor particle number for each particle 
    unsigned N0 = 0;//Max neiboor particle number
    #ifdef OMP_USE
        #pragma omp parallel for schedule (guided)
    #endif

    for (int p1 = 0; p1 < pinit; p1++) {
        pardet[p1] = 4;
    }
    #ifdef OMP_USE
        #pragma omp parallel for schedule (guided)
    ...
    for (int p1 = int(pinit); p1 < pfin; p1++) {
        ...
        float div = 0.0;
        unsigned np2inp1 = 0;
        //-Search for neighbours fluid particle in adjacent cells.
        bool boundp2 = false;
        StNgSearch ngs = nsearch::Init(dcell[p1], boundp2, divdata);
        for (int z = ngs.zini; z < ngs.zfin; z++)for (int y = ngs.yini; y < ngs.yfin; y++) {
            const tuint2 pif = nsearch::ParticleRange(y, z, ngs, divdata);
            ...
            for (unsigned p2 = pif.x; p2 < pif.y; p2++) {
                ...
                if (rr2 <= KernelSize2 && rr2 >= ALMOSTZERO) {
                    ...
                    float massp2 = (boundp2 ? MassBound : MassFluid);
                    float volp2 = massp2 / velrhop[p2].w;
                    div += -(drx * frx + dry * fry + drz * frz) * volp2;
                    np2inp1 += 1;
                }
            }
        }
        //-Search for neighbours bound particle in adjacent cells.
        boundp2 = true;
        ngs = nsearch::Init(dcell[p1], boundp2, divdata);
        for (int z = ngs.zini; z < ngs.zfin; z++)for (int y = ngs.yini; y < ngs.yfin; y++) {
            const tuint2 pif = nsearch::ParticleRange(y, z, ngs, divdata);
        ...
        #pragma omp parallel for schedule (guided)
    #endif
    //-Particle type 0-splash 1-free surface 2-free surface vicinity 3-inner 4-wall vicinity
    //-Step 1 splash & cadidate & inner
    for (int p1 = int(pinit); p1 < pfin; p1++) {
        if (posdiv[p1] < 0.4 && Ni[p1] < 0.4 * N0)
            pardet[p1] = 0;
        else if (posdiv[p1] >= 0.4 && posdiv[p1] < 1.75)
            pardet[p1] = 1;
        else
            pardet[p1] = 3;
    }
        ...
        #pragma omp parallel for schedule (guided)
    #endif
    //  step 2 find free surface p in f cadidate
    for (int p1 = int(pinit); p1 < pfin; p1++)
    {
        if (pardet[p1] == 1) {
            const tdouble3 posp1 = pos[p1];
            ...
            for (int z = ngs.zini; z < ngs.zfin; z++)for (int y = ngs.yini; y < ngs.yfin; y++) {
                ...
                for (unsigned p2 = pif.x; p2 < pif.y; p2++) {
                    ...
                    if (rr2 <= KernelSize2 && rr2 >= ALMOSTZERO) {
                        ...
                        const float rij = sqrt(rr2);
                        const float vecProduct = (-drx * parnormal[p1].x + -dry * parnormal[p1].y
                            + -drz * parnormal[p1].z) / rij;
                        const bool bool1 = (rij<KernelSize * sqrt(2) && vecProduct>cos(45 * 3.14159265 / 180));

                        float rTi_x = KernelH * parnormal[p1].x;
                        float rTi_y = KernelH * parnormal[p1].y;
                        float rTi_z = KernelH * parnormal[p1].z;
                        const float velLength = sqrt((drx + rTi_x) * (drx + rTi_x) +
                            (drz + rTi_z) * (drz + rTi_z));
                        const bool bool2 = (rij >= KernelH * sqrt(2) && velLength < KernelH);
                        if (bool1 || bool2) {
                            pardet[p1] = 3;
                            break;
                        }
        ...
        #pragma omp parallel for schedule (guided)
    #endif
    //-Step 3 find f vicinity in inner 
    for (int p1 = int(pinit); p1 < pfin; p1++)
    {
        if (pardet[p1] == 3)
        {
            ...
            for (int z = ngs.zini; z < ngs.zfin; z++)for (int y = ngs.yini; y < ngs.yfin; y++) {
                ...
                for (unsigned p2 = pif.x; p2 < pif.y; p2++)
                {
                    if (pardet[p2] == 1)
                    {
                        ...
                        rjimin = min(rjimin, rr);
                    }
                }
            }

            if (rjimin < 2 * KernelH - 0.5 * Dp) {
                pardet[p1] = 2;
            }
        ...
        #pragma omp parallel for schedule (guided)
    #endif
    //-Step 4 find wall vicinity particle
    for (int p1 = int(pinit); p1 < pfin; p1++)
    {
        if (pardet[p1] == 1 || pardet[p1] == 2)
        {
            const tdouble3 posp1 = pos[p1];
            bool boundp2 = true;
            float rjimin = FLT_MAX;
...
void JSphCpu::CalDiv(const stinterparmsc& t)const {
   if (TKernel == KERNEL_Wendland)  CalDivFin<KERNEL_Wendland>(t.np, 0, t.divdata, t.dcell, t.pos, t.velrhop, t.code, t.idp);
   else if (TKernel == KERNEL_Cubic) CalDivFin<KERNEL_Cubic>(t.np, 0, t.divdata, t.dcell, t.pos, t.velrhop, t.code, t.idp);
}
//
template<TpKernel tker> void JSphCpu::CalDivFin(unsigned n, unsigned pinit, StDivDataCpu divdata, const unsigned* dcell,
   const tdouble3* pos, const tfloat4* velrhop, const typecode* code, const unsigned* idp) const {
   ...
   for (int p1 = int(pinit); p1 < pfin; p1++) {
       ...
       for (int z = ngs.zini; z < ngs.zfin; z++)for (int y = ngs.yini; y < ngs.yfin; y++) {
           ...
           for (unsigned p2 = pif.x; p2 < pif.y; p2++) {
               ...
               if (rr2 <= KernelSize2 && rr2 >= ALMOSTZERO) {
                   ...
                   //-Velocity divergence (Continuity equation).
                   const float dvx = velp1.x - velrhop2.x, dvy = velp1.y - velrhop2.y, dvz = velp1.z - velrhop2.z;
                   veldivp1 += -massp2 * (dvx * frx + dvy * fry + dvz * frz) / velrhop2.w;
                   rhooverp1 += massp2 * wab;
               }
           }
       }
       ...
       VelDivc[p1] = veldivp1;
       RhoOver[p1] = (rhooverp1+MassFluid* fsph::GetKernel_Wab<tker>(CSP, 0))/ rhoref;
   }
};

template<TpKernel tker,bool shift> void JSphCpu::ShiftFluid(unsigned n, unsigned pini
    , StDivDataCpu divdata, const unsigned* dcell, const tdouble3* pos, const tfloat4* velrhop
    , tfloat4* shiftposfs, const tmatrix2d* corrmat, const unsigned* pardet, const tfloat3* normalvec)const
{

    // Get Shifting Value by particle interacion and then multiply by some coefficient in JSphShifting::RunCpuOps
    const int pfin = int(pini + n);

    float* disforFv = new float[pfin]();  //min distance of Free surface vicinity particle and Free S particle 
    unsigned* p2forFv = new unsigned[pfin]();//index 
    ...
    for (int p1 = int(pini); p1 < pfin; p1++) {
        ...
        for (int z = ngs.zini; z < ngs.zfin; z++)for (int y = ngs.yini; y < ngs.yfin; y++) {
            ...
            for (unsigned p2 = pif.x; p2 < pif.y; p2++) {
                ...
                if (rr2 <= KernelSize2 && rr2 >= ALMOSTZERO) {
                    ...
                    const float fac = fsph::GetKernel_Fac<tker>(CSP, rr2);
                    const float wab = fsph::GetKernel_Wab<tker>(CSP, rr2);
                    const float wab0 = fsph::GetKernel_Wab<tker>(CSP, Dp * Dp);
                    const float frx = fac * drx, fry = fac * dry, frz = fac * drz; //-Gradients.
                    float massp2 = (boundp2 ? MassBound : MassFluid); //-Contiene masa de particula segun sea bound o fluid.
                    ...
                    // Get max cos degree
                    if (pardet[p1] == 1 && pardet[p2] == 1)
                    {
                        float arcos = acos(normalvec[p1].x * normalvec[p2].x + normalvec[p1].z * normalvec[p2].z);
                        maxarcos = (maxarcos > arcos ? maxarcos : arcos);
                    }
                    ...
                    if (pardet[p1] == 1)
                    {
                        shiftposfsp1.x += 1 * massrhop * frx * (0.2 * pow(wab / wab0, 4));
                        shiftposfsp1.y += 1 * massrhop * fry * (0.2 * pow(wab / wab0, 4));
                        shiftposfsp1.z += 1 * massrhop * frz * (0.2 * pow(wab / wab0, 4));
                    }
                    else if (pardet[p1] == 3)
                    {
                        shiftposfsp1.x += massrhop * frx * (1 + 0.2 * pow(wab / wab0, 4));
                        shiftposfsp1.y += massrhop * fry * (1 + 0.2 * pow(wab / wab0, 4));
                        shiftposfsp1.z += massrhop * frz * (1 + 0.2 * pow(wab / wab0, 4));
                    }
                    else if (pardet[p1] == 2)
                    {
                        double phi = (p2forFv[p1] != FLT_MAX) ? ((pos[p2forFv[p1]].x - posp1.x) * normalvec[p2forFv[p1]].x
                            + (pos[p2forFv[p1]].z - posp1.z) * normalvec[p2forFv[p1]].z) /
                            sqrt((pos[p2forFv[p1]].x - posp1.x) * (pos[p2forFv[p1]].x - posp1.x) +
                                (pos[p2forFv[p1]].z - posp1.z) * (pos[p2forFv[p1]].z - posp1.z)) : 0;
                        phi = phi > 0 ? phi : 0; // coefficient > 0 for free surface vicinity particles
                        shiftposfsp1.x += phi * massrhop * frx * (0.2 * pow(wab / wab0, 4));
                        shiftposfsp1.y += phi * massrhop * fry * (0.2 * pow(wab / wab0, 4));
                        shiftposfsp1.z += phi * massrhop * frz * (0.2 * pow(wab / wab0, 4));
                    }
                    else
                    {
                        shiftposfsp1 = TFloat4(0);
                    }
                }
            }
        }
        //-Search for boundary particles 
        boundp2 = true;
        //-Search for neighbours in adjacent cells.
        ngs = nsearch::Init(dcell[p1], boundp2, divdata);
        for (int z = ngs.zini; z < ngs.zfin; z++)for (int y = ngs.yini; y < ngs.yfin; y++) {
        ...
        //-Free surface ops process
        if (pardet[p1] == 1)
        {
            tfloat3 shiftp = TFloat3(0);
            shiftp.x = (1 - double(normalvec[p1].x) * double(normalvec[p1].x)) * shiftposfs[p1].x
                - (double(normalvec[p1].x) * double(normalvec[p1].z)) * shiftposfs[p1].z;
            shiftp.y = 0.0;
            shiftp.z = -(double(normalvec[p1].x) * double(normalvec[p1].z)) * shiftposfs[p1].x
                + (1 - double(normalvec[p1].z) * double(normalvec[p1].z)) * shiftposfs[p1].z;
            shiftposfs[p1].x = shiftp.x;
            shiftposfs[p1].y = shiftp.y;
            shiftposfs[p1].z = shiftp.z;
        }

        if (maxarcos > PI / 6 && (pardet[p1] == 1)) {
            shiftposfs[p1] = TFloat4(0);
        }
...
void JSphCpu::RunVEMVCS_StepI(const stvemvcsparmsc& t, double dt)const {
    if (TKernel == KERNEL_Wendland) RunVEMVCS_StepI_Fin<KERNEL_Wendland>(t.vem, t.vcs, t.np, t.npb, t.divdata, t.dcell, t.pos, t.velrhop, t.pardet, t.pressvem, t.pressvcs, dt);
    else if (TKernel == KERNEL_Cubic) RunVEMVCS_StepI_Fin<KERNEL_Cubic>(t.vem, t.vcs, t.np, t.npb, t.divdata, t.dcell, t.pos, t.velrhop, t.pardet, t.pressvem, t.pressvcs, dt);
}

//-VEM pressure and VCS pressure
template<TpKernel tker> void JSphCpu::RunVEMVCS_StepI_Fin(bool vem, bool vcs, unsigned n, unsigned pinit, StDivDataCpu divdata, const unsigned* dcell,
    const tdouble3* pos, const tfloat4* velrhop, const unsigned* pardet, float* pressvem, float* pressvcs, double dt) const {

    //bool boundp2;
    const int pini = int(pinit);
    const int pfin = int(n);
    //================================ VEM STEP=====================================
    //-Note in CPU code p^VEM and p^VCS are not computed in one loop for code readability. In the GPU version, they are computed in the same loop to improve efficiency. 
    if(vem){
        #ifdef OMP_USE
            #pragma omp parallel for schedule (guided)
        #endif
        for (int p1 = pini; p1 < pfin; p1++) {
            ...
            pressvem[p1] = Cs0 * Cs0 * dt * arp1;//-P^{VEM} from divergence of Vel
        }
    }
    ...
    if(vcs){
        ...
        // - Arrays used to get VCS Pressure
        float* rhobysum = new float[pfin](); //rho_i_bar
        float* pressnew = new float[pfin](); //pressvcs at timestep = k+1
        float* Aij = new float[pfin]();      //Aij E.Q.(53) IN AMM paper
        float* PAij = new float[pfin]();     //PAij E.Q.(53) IN AMM paper
        float* disforFv = new float[pfin]();//distance between free surface vicinity particle  with freesurface and inner
        unsigned* disminindex = new unsigned[pfin]();//min distance particle index 
        float* wabsum = new float[pfin]();// kernel wab summation
        float* velDiv = new float[pfin]();// divergence of velocity
        ...
        for (int p1 = pini; p1 < pfin; p1++) {
            ...
            const tdouble3 posp1 = pos[p1];
            const float rhopp1 = velrhop[p1].w;
            float rhop1 = 0.0; float aij = 0.0; float paij = 0.0;

            //- Find nearby fluid particles
            StNgSearch ngs = nsearch::Init(dcell[p1], false, divdata);
            for (int z = ngs.zini; z < ngs.zfin; z++)for (int y = ngs.yini; y < ngs.yfin; y++) {
                const tuint2 pif = nsearch::ParticleRange(y, z, ngs, divdata);
                for (unsigned p2 = pif.x; p2 < pif.y; p2++) {
                    ...
                    if (rr2 <= KernelSize2 && rr2 >= ALMOSTZERO) {
                        ...
                        const float fac = fsph::GetKernel_Fac<tker>(CSP, rr2);
                        const float wab = fsph::GetKernel_Wab<tker>(CSP, rr2);
                        /*const float wab0 = fsph::GetKernel_Wab<tker>(CSP, 0);*/
                        const float frx = fac * drx, fry = fac * dry, frz = fac * drz; //-Gradients.
                        boundp2 = false;
                        float massp2 = (boundp2 ? MassBound : MassFluid);
                        // rho sum
                        //rhop1 += massp2 * wab / wabsum[p1];
                        rhop1 += massp2 * wab;
                        // Aij sum
                        aij += 8 * massp2 / (rhopp1 + rhopp2) / (rhopp1 + rhopp2)
                            * (-1.0) * (drx * frx + dry * fry + drz * frz) / rr2;
                        // PAij sum
                        paij += 8 * massp2 / (rhopp1 + rhopp2) / (rhopp1 + rhopp2)
                            * (-1.0) * (drx * frx + dry * fry + drz * frz) / rr2 * pvcsp2;
                    }
                }
            }

            //- Find nearby bound particles
            ngs = nsearch::Init(dcell[p1], true, divdata);
            for (int z = ngs.zini; z < ngs.zfin; z++)for (int y = ngs.yini; y < ngs.yfin; y++) {
                const tuint2 pif = nsearch::ParticleRange(y, z, ngs, divdata);
                for (unsigned p2 = pif.x; p2 < pif.y; p2++) {
            ...
            rhobysum[p1] = rhop1 + MassFluid * fsph::GetKernel_Wab<tker>(CSP, 0.0);
            Aij[p1] = aij;
            PAij[p1] = paij;
        }

        // =====================================================================
        //   min dis between free surface and inner for free-surface vicinity
        // =====================================================================
        #ifdef OMP_USE
            #pragma omp parallel for schedule (guided)
        #endif
        for (int p1 = int(pini); p1 < pfin; p1++) {
            float dismininner = FLT_MAX, disminp2 = FLT_MAX;
            const tdouble3 posp1 = pos[p1];
            if (pardet[p1] == 2) {
                StNgSearch ngs = nsearch::Init(dcell[p1], false, divdata);
                for (int z = ngs.zini; z < ngs.zfin; z++)for (int y = ngs.yini; y < ngs.yfin; y++) {
                    ...
                    for (unsigned p2 = pif.x; p2 < pif.y; p2++) {
                        ...
                        if (rr2 <= KernelSize2 && rr2 >= ALMOSTZERO) {
                            ...
                            {
                                float dis = sqrt(rr2);
                                dismininner = (dismininner > dis ? dis : dismininner);
                                if (dismininner == dis)
                                {
                                    disminp2 = p2;
                                }
                ...
                disforFv[p1] = dismininner;
                disminindex[p1] = disminp2;
            }
        }
        ...
        for (int p1 = pini; p1 < pfin; p1++) {
            ...
            // used for vcs
            const float cs0 = CSP.cs0;
            const float rho0 = rhoref;//reference density
            const float rhopbysump1 = rhobysum[p1];
            //const float bivcs = (-velDiv[p1]) / (dt);
            const float divcs = 1 / (rho0 * dt * dt * cs0 * cs0);
            const float eivcs = -(rho0 - rhopbysump1) / (rho0 * dt * dt);
            const float pressfin = (PAij[p1] + eivcs) / (Aij[p1] + divcs);
            //SourceDiC[p1] = eivcs;
            pressnew[p1] = pressfin;
        }
        #ifdef OMP_USE
            #pragma omp parallel for schedule (guided)
        #endif
        // Vcs pressure for vicinity particle interpolation
        for (int p1 = pini; p1 < pfin; p1++) {
            if (pardet[p1] == 0 || pardet[p1] == 1 || pardet[p1] == 4)pressnew[p1] = 0;
            if (pardet[p1] == 2 && disminindex[p1] != FLT_MAX) {
                pressnew[p1] = pressnew[disminindex[p1]]
                    * (2 * KernelH - disforFv[p1]) / (2 * KernelH);
            }
        }
        ...
        for (int p1 = 0; p1 < pini; p1++)
        {
            ...
            if (kernelSum != 0)
            {
                pressvcsP1 = pressvcsP1 / kernelSum;
            }
            pressnew[p1] = pressvcsP1;
        }

        //============================================================
        //========================Vcs k+1 from pressnew===============
        //============================================================
        memcpy(pressvcs, pressnew, sizeof(float) * pfin);
...
void JSphCpu::RunVEMVCS_StepII(const stvemvcsparmsc& t, double dt)const {
    if (TKernel == KERNEL_Wendland) RunVEMVCS_StepII_Fin<KERNEL_Wendland>(t.vem, t.vcs, t.np, t.npb, t.divdata, t.dcell, t.pos, t.velrhop, t.corrmat, t.pardet, t.pressvem, t.acevem, t.pressvcs, t.shiftvcs, dt);
    else if (TKernel == KERNEL_Cubic) RunVEMVCS_StepII_Fin<KERNEL_Cubic>(t.vem, t.vcs, t.np, t.npb, t.divdata, t.dcell, t.pos, t.velrhop, t.corrmat, t.pardet, t.pressvem, t.acevem, t.pressvcs, t.shiftvcs, dt);
}

//-VEM acceleration and VCS shifting vector
template<TpKernel tker> void JSphCpu::RunVEMVCS_StepII_Fin(const bool vem, const bool vcs, unsigned n, unsigned pinit, StDivDataCpu divdata, const unsigned* dcell,
    const tdouble3* pos, const tfloat4* velrhop, const tmatrix2d* corrmat, const unsigned* pardet, float* pressvem, tdouble3* acevem, float* pressvcs
    , tdouble3* shiftvcs, double dt) const {
    //bool boundp2;
    const int pini = int(pinit);
    const int pfin = int(n);
    #ifdef OMP_USE
    ...
    for (int p1 = int(pinit); p1 < pfin; p1++) {
        bool boundp2;
        tfloat3 acevemp1 = TFloat3(0);
        const float rhopp1 = velrhop[p1].w;
        const tdouble3 posp1 = pos[p1];

        //-Search for neighbours Fluid particles in adjacent cells.
        boundp2 = false;
        StNgSearch ngs = nsearch::Init(dcell[p1], boundp2, divdata);
        //-Search for neighbours in adjacent cells.
        for (int z = ngs.zini; z < ngs.zfin; z++)for (int y = ngs.yini; y < ngs.yfin; y++) {
            ...
            for (unsigned p2 = pif.x; p2 < pif.y; p2++) {
                ...
                if (rr2 <= KernelSize2 && rr2 >= ALMOSTZERO) {
                    ...
                    if (compute) {
                        if(vem){
                            if (pressvem[p1] <= 0 && pardet[p1] == 3) {
                                const float prs = (pressvem[p2] - pressvem[p1]) / (rhopp1 * velrhop2.w);
                                const float p_vpm = -prs * massp2;
                                acevemp1.x += p_vpm * frx; acevemp1.y += p_vpm * fry; acevemp1.z += p_vpm * frz;
                            }
                            else {
                                const float prs = (pressvem[p1] + pressvem[p2]) / (rhopp1 * velrhop2.w);
                                const float p_vpm = -prs * massp2;
                                acevemp1.x += p_vpm * frx; acevemp1.y += p_vpm * fry; acevemp1.z += p_vpm * frz;
                            }
                            //-VEM without TIC
                            //const float prs = (pressvemp1 + pressvem[p2]) / (rhopp1 * velrhop2.w);
                            //const float p_vpm = -prs * massp2;
                            //acevemp1.x += p_vpm * frx; acevemp1.y += p_vpm * fry; acevemp1.z += p_vpm * frz;
                        }
                        if(vcs){
                            double pconst = -dt * dt / rhopp1 * (pressvcs[p2] - pressvcs[p1]) * massp2 / velrhop2.w;
                            shiftvcs[p1].x += pconst * (corrmat[p1].a11 * frx + corrmat[p1].a12 * frz);
                            shiftvcs[p1].z += pconst * (corrmat[p1].a21 * frx + corrmat[p1].a22 * frz);
                        }
    ...
    //-VCS shift vector
    //-Set vcs=0 for splash particles and max vcs shifting = h/20 
    if(vcs){
        #ifdef OMP_USE
            #pragma omp parallel for schedule (guided)
        #endif
        for (int p1 = pini; p1 < pfin; p1++) {
            ...
            // Max shift value h/20
            double shifvcsmag = sqrt(shiftvcs[p1].x * shiftvcs[p1].x + shiftvcs[p1].z * shiftvcs[p1].z);
            shiftvcs[p1].x = (shifvcsmag < KernelH / 20.0) ? shiftvcs[p1].x : KernelH / 20.0 * shiftvcs[p1].x / shifvcsmag;
            shiftvcs[p1].z = (shifvcsmag < KernelH / 20.0) ? shiftvcs[p1].z : KernelH / 20.0 * shiftvcs[p1].z / shifvcsmag;
        }
    }
...
void JSphCpu::RunVemPress_t1(const stvemparmsc& t, double dt)const {
    if (TKernel == KERNEL_Wendland)  RunVemPress_Fin<KERNEL_Wendland>(t.np, t.npb, t.divdata, t.dcell, t.pos, t.velrhop,t.pressvem,dt);
    else if (TKernel == KERNEL_Cubic) RunVemPress_Fin<KERNEL_Cubic>(t.np, t.npb, t.divdata, t.dcell, t.pos, t.velrhop,t.pressvem,dt);
}
//
template<TpKernel tker> void JSphCpu::RunVemPress_Fin(unsigned n, unsigned pinit, StDivDataCpu divdata, const unsigned* dcell,
    const tdouble3* pos, const tfloat4* velrhop,float *pressvem, double dt) const {
...
void JSphCpu::VemPressInteraction_t1(const stvemcorrparmsc& t)const {
    if (TKernel == KERNEL_Wendland)  VemPressInteraction_Fin<KERNEL_Wendland>(t.np, t.npb, t.divdata, t.dcell, t.pos
        , t.velrhop, t.pressvem,t.pardet,t.acevem);
    else if (TKernel == KERNEL_Cubic) VemPressInteraction_Fin<KERNEL_Cubic>(t.np, t.npb, t.divdata, t.dcell, t.pos
        , t.velrhop, t.pressvem, t.pardet, t.acevem);
}
//
template<TpKernel tker> void JSphCpu::VemPressInteraction_Fin(unsigned n, unsigned pinit, StDivDataCpu divdata, const unsigned* dcell,
    const tdouble3* pos, const tfloat4* velrhop, const float* pressvem, const unsigned* pardet, tdouble3* acevem) const {
    ...
    for (int p1 = int(pinit); p1 < pfin; p1++) {
        ...
        tfloat3 acevemp1 = TFloat3(0);
        const float rhopp1 = velrhop[p1].w;
        const float pressvemp1 = pressvem[p1];
        const tdouble3 posp1 = pos[p1];
        ...
        for (int z = ngs.zini; z < ngs.zfin; z++)for (int y = ngs.yini; y < ngs.yfin; y++) {
            ...
            for (unsigned p2 = pif.x; p2 < pif.y; p2++) {
                ...
                if (rr2 <= KernelSize2 && rr2 >= ALMOSTZERO) {
                    ...
                    if (compute) {
                        if (pressvemp1 <= 0 && pardet[p1] == 3) {
                            const float prs = (pressvem[p2] - pressvemp1) / (rhopp1 * velrhop2.w);
                            const float p_vpm = -prs * massp2;
                            acevemp1.x += p_vpm * frx; acevemp1.y += p_vpm * fry; acevemp1.z += p_vpm * frz;
                        }
                        else {
                        ...
                        // riemann for vem
                        //tdouble3 eij = TDouble3(-drx, 0, -drz) / double(sqrt(rr2));
                        //const double ul = velrhop[p1].x * eij.x + velrhop[p1].z * eij.z, ur = velrhop2.x * eij.x + velrhop2.z * eij.z;
                        //const double pl = pressvemp1, pr = pressvem[p2];
                        //const double cl = Cs0, cr = Cs0;
                        ////const double rhol = rhopp1, rhor = velrhop2.w;
                        //const double ubar = 0.5 * (ul + ur), pbar = 0.5 * (pl + pr), rhobar = 0.5 * (rhopp1 + velrhop2.w);
                        //const double lamda = 3.0;
                        //const double beta = min(lamda * max(ul - ur, 0), Cs0);
                        //const double pstar = pbar + 0.5 * beta * (rhopp1 + velrhop2.w) / 2.0 * (ul - ur);
                        //const double p_vpm = -2.0 * massp2 * pstar / (rhopp1 * velrhop2.w);
                        //acevemp1.x += p_vpm * frx; acevemp1.y += p_vpm * fry; acevemp1.z += p_vpm * frz;
                    }
                }
...
//============================= VCS STEP=====================================
//===========================================================================
void JSphCpu::VcsStep(const stvcsparmsc& t, double dt)const {
    if (TKernel == KERNEL_Wendland)  Vcs_ct2<KERNEL_Wendland>(t, dt);
    else if (TKernel == KERNEL_Cubic) Vcs_ct2<KERNEL_Cubic   >(t, dt);
}

template<TpKernel tker>
void JSphCpu::Vcs_ct2(const stvcsparmsc& t, double dt)const
{
    RunVcsPress<tker>(t.npf, t.npb, t.divdata, t.dcell
        , t.pos, t.velrhop, t.press, t.pressvcs, t.pardet, dt);
    RunVcsShift<tker>(t.npf, t.npb, t.divdata, t.dcell, t.pos, t.velrhop
        , t.pressvcs, dt, t.shiftvcs, t.corrmat);
}

//==========A. Khayyer Applied Mathematical Modelling (2022) 84-121.============
//==============================================================================
//================================ VCS STEP=====================================
//==============================================================================
template<TpKernel tker>void JSphCpu::RunVcsPress(
    unsigned n, unsigned pini, StDivDataCpu divdata, const unsigned* dcell
    , const tdouble3* pos, const tfloat4* velrhop, const float* press
    , float* pressvcs, const unsigned* pardet, double dt
)const
{
    const int pfin = int(pini + n);
    ...
    // Vcs pressure for vicinity particle interpolation
    for (int p1 = pini; p1 < pfin; p1++) {
        if (pardet[p1] == 0 || pardet[p1] == 1)pressnew[p1] = 0;
        if (pardet[p1] == 2 && disminindex[p1] != FLT_MAX) {
            pressnew[p1] = pressnew[disminindex[p1]]
                * (2 * KernelH - disforFv[p1]) / (2 * KernelH);
        }
...
template<TpKernel tker> void JSphCpu::RunVcsShift(unsigned n, unsigned pini, StDivDataCpu divdata
    , const unsigned* dcell, const tdouble3* pos, const tfloat4* velrhop
    , const float* pressvcs, const double dt, tdouble3* shiftvcs, const tmatrix2d* corrmat
)const
{
    const int pfin = int(pini + n);
    // ==========================================================
        ...
        for (int z = ngs.zini; z < ngs.zfin; z++)for (int y = ngs.yini; y < ngs.yfin; y++) {
            ...
            for (unsigned p2 = pif.x; p2 < pif.y; p2++) {
                ...
                if (rr2 <= KernelSize2 && rr2 >= ALMOSTZERO) {
                    ...
                    const float frx = fac * drx, fry = fac * dry, frz = fac * drz; //-Gradients.
                    tfloat4 velrhop2 = velrhop[p2];
                    double pconst = -dt * dt / rhopp1 * (pressvcsp2 - pressvcsp1) * massp2 / velrhop2.w;
                    shiftvcs[p1].x += pconst * (corrmat[p1].a11 * frx + corrmat[p1].a12 * frz);
                    shiftvcs[p1].z += pconst * (corrmat[p1].a21 * frx + corrmat[p1].a22 * frz);
                }
            }
...
// Get reference density using max m_i
void JSphCpu::GetRhoRef(const stinterparmsc& t) {
    if (TKernel == KERNEL_Wendland)  GetRhoFin<KERNEL_Wendland>(t.np, t.npb, t.divdata, t.dcell, t.pos, t.velrhop, t.code, t.idp);
    else if (TKernel == KERNEL_Cubic) GetRhoFin<KERNEL_Cubic>(t.np, t.npb, t.divdata, t.dcell, t.pos, t.velrhop, t.code, t.idp);
}

template<TpKernel tker> void JSphCpu::GetRhoFin(unsigned n, unsigned pinit, StDivDataCpu divdata, const unsigned* dcell,
    const tdouble3* pos, const tfloat4* velrhop, const typecode* code, const unsigned* idp) {
    ...
    for (int p1 = int(pinit); p1 < pfin; p1++) {
        ...
        double massp1 = CODE_IsFluid(rcode) ? MassFluid : MassBound;
        double mi = 0.0; // mi = sum_mjwij
        //-Obtain data of particle p1.
        const tdouble3 posp1 = pos[p1];
        const double rhopp1 = velrhop[p1].w;
        //-Search for neighbours in adjacent cells.
        boundp2 = false;
        StNgSearch ngs = nsearch::Init(dcell[p1], boundp2, divdata);
        for (int z = ngs.zini; z < ngs.zfin; z++)for (int y = ngs.yini; y < ngs.yfin; y++) {
            ...
            for (unsigned p2 = pif.x; p2 < pif.y; p2++) {
                ...
                double dry = double(posp1.y - pos[p2].y);
                if (rsym)    dry = double(posp1.y + pos[p2].y); //<vs_syymmetry>
                const double drz = double(posp1.z - pos[p2].z);
                const double rr2 = drx * drx + dry * dry + drz * drz;
                if (rr2 <= KernelSize2 && rr2 >= ALMOSTZERO) {
                    //-Computes kernel.
                    const double wab = fsph::GetKernel_Wab<tker>(CSP, rr2);
                    //===== Get mass of particle p2 ===== 
                    double massp2 = (boundp2 ? MassBound : MassFluid); //-Contiene masa de particula segun sea bound o fluid.
        ...
        mi += massp1 * fsph::GetKernel_Wab<tker>(CSP, 0);
        rhoref = rhoref < mi ? mi : rhoref;
    }
}
...
void JSphCpu::ComputeSymplecticPre(double dt) {
    ...
    if (HpdcAdded)
    {
        PsiPrec = ArraysCpu->ReserveFloat();
        swap(PsiPrec, Psic);
    }
    ...
    for (int p = npb; p < np; p++) {
        ...
        if (HpdcAdded)Psic[p] = PsiPrec[p] + dt05 * APsi[p];
...
void JSphCpu::ComputeSymplecticCorr(double dt) {
    ...
    for (int p = npb; p < np; p++) {
        ...
        const double epsilon_rdot = (-double(Arc[p]) / double(Velrhopc[p].w)) * dt;
        const float rhopnew = float(double(VelrhopPrec[p].w) * (2. - epsilon_rdot) / (2. + epsilon_rdot));
        if (HpdcAdded)Psic[p] = PsiPrec[p] + double(APsi[p] * dt); //-Update \Psi
        if (!WithFloating || CODE_IsFluid(rcode)) {//-Fluid Particles.
            //-VEM acceleration added here
            if(VemAdded){
                ...
                Acec[p].x += absacevem > maxacevem ? maxacevem * AceVemc[p].x / absacevem : AceVemc[p].x;
                Acec[p].y += absacevem > maxacevem ? maxacevem * AceVemc[p].y / absacevem : AceVemc[p].y;
                Acec[p].z += absacevem > maxacevem ? maxacevem * AceVemc[p].z / absacevem : AceVemc[p].z;
            }
            //-Calculate velocity & density. | Calcula velocidad y densidad.
            tfloat4 rvelrhopnew = TFloat4(
    ...
    if (HpdcAdded) { ArraysCpu->Free(PsiPrec); PsiPrec = NULL; }
    Timersc->TmStop(TMC_SuComputeStep);
}
