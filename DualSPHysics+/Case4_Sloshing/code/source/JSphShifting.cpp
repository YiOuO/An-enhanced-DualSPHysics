...
void JSphShifting::RunCpuOps(unsigned n, unsigned pini, double dt, const tfloat4* velrhop
    , tfloat4* shiftposfs,const float * dt_i,const float c_ad)const
{
    const double coefumagn = -0.5 * KernelH * KernelH;
    const float maxdist = float(KernelH/20); //-Max shifting distance permitted (recommended).
    const int ppini = int(pini), ppfin = pini + int(n), npf = int(n);
    // umax 
    #ifdef OMP_USE
        #pragma omp parallel for schedule (static) if(npf>OMP_LIMIT_COMPUTELIGHT)
    ...
    for (int p = ppini; p < ppfin; p++) {
        const tfloat4 rs = shiftposfs[p];
        if (rs.x != FLT_MAX) {
            double umagn = 1.0*coefumagn*dt/(c_ad*dt_i[p]); // coef in e.q(30) in AMM paper
            //double umagn = coefumagn ;
            const double shiftdistx = double(double(rs.x) * umagn);
            const double shiftdisty = double(double(rs.y) * umagn);
            const double shiftdistz = double(double(rs.z) * umagn);

            const double shiftall = sqrt(shiftdistx * shiftdistx + shiftdisty * shiftdisty + shiftdistz * shiftdistz);

            //shiftposfs[p].x = (fabs(shiftdistx) < maxdist ? shiftdistx : (shiftdistx >= 0 ? maxdist : -maxdist));
            //shiftposfs[p].y = (fabs(shiftdisty) < maxdist ? shiftdisty : (shiftdisty >= 0 ? maxdist : -maxdist));
            //shiftposfs[p].z = (fabs(shiftdistz) < maxdist ? shiftdistz : (shiftdistz >= 0 ? maxdist : -maxdist));
            shiftposfs[p].x = (shiftall < maxdist) ? shiftdistx : maxdist * shiftdistx / shiftall;
            shiftposfs[p].y = (shiftall < maxdist) ? shiftdisty : maxdist * shiftdisty / shiftall;
            shiftposfs[p].z = (shiftall < maxdist) ? shiftdistz : maxdist * shiftdistz / shiftall;
