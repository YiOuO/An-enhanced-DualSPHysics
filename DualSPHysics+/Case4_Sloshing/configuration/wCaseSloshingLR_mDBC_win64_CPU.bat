...
%partvtk% -dirin %diroutdata% -savevtk %dirout2%/PartMoving -onlytype:-all,+moving -vars:+idp,+vel,+rhop,+press,+VelDiv,+NormalVec,+ParType,+Rho_Rho0,+Psi
if not "%ERRORLEVEL%" == "0" goto fail

%partvtk% -dirin %diroutdata% -savevtk %dirout2%/PartFluid -onlytype:-all,+fluid -vars:+idp,+vel,+rhop,+press,+VelDiv,+NormalVec,+ParType,+Rho_Rho0,+Psi
if not "%ERRORLEVEL%" == "0" goto fail
