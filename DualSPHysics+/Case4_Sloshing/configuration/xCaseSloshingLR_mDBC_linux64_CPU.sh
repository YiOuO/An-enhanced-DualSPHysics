...
${partvtk} -dirin ${diroutdata} -savevtk ${dirout2}/PartMoving -onlytype:-all,+moving -vars:+idp,+vel,+rhop,+press,+VelDiv,+ParType,+Rho_Rho0
if [ $? -ne 0 ] ; then fail; fi

${partvtk} -dirin ${diroutdata} -savevtk ${dirout2}/PartFluid -onlytype:-all,+fluid -vars:+idp,+vel,+rhop,+press,+VelDiv,+ParType,+Rho_Rho0
if [ $? -ne 0 ] ; then fail; fi
...
# Executes BoundaryVTK to create a CSV file with positions where pressure is measured using MeasureTool.
${boundaryvtk} -motiondata ${diroutdata} -saveposmotion:10:-0.45:0:0.093 ${dirout2}/PosCorrect 
if [ $? -ne 0 ] ; then fail; fi
# Executes MeasureTool to create a CSV file with pressure information at point close to the boundary.
${measuretool} -dirin ${diroutdata} -pointspos ${dirout2}/PosCorrect_mk0010.csv -onlytype:-all,+fluid,+bound -vars:-all,+press,+kcorr -kcusedummy:0 -kclimit:0.5 -savevtk ${dirout2}/PointsPressure_Correct_002 -savecsv ${dirout2}/_PointsPressure_Correct_002
if [ $? -ne 0 ] ; then fail; fi
